...
func TestSyncCurrentWorldToConnIncludesBuildingConfigs(t *testing.T) {
	...
	syncCurrentWorldToConn(srv, sendConn, w)
...
func TestSendBlockSnapshotsToConnEmitsBlockSnapshotPayload(t *testing.T) {
	...
	sendBlockSnapshotsToConn(srv, sendConn, w)
...
func TestSendBlockSnapshotsToConnFiltersHiddenEnemySnapshotsByViewerTeam(t *testing.T) {
	...
	sendBlockSnapshotsToConn(srv, enemyConn, w)
	...
	sendBlockSnapshotsToConn(srv, allyConn, w)
...
func TestSendRequestedBlockSnapshotToConnEmitsOnlyBlockSnapshotPayload(t *testing.T) {
	...
	sendRequestedBlockSnapshotToConn(srv, sendConn, w, containerPos)
	packets := collectRawPacketsUntilTimeout(t, clientSide, 4)
	if len(packets) != 1 {
		descriptions := make([]string, 0, len(packets))
		for _, packet := range packets {
