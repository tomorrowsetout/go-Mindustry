...
func EnsureWorkspace(cfgPath string, cfg config.Config) (Result, error) {
	...
	_ = writeIfMissing(filepath.Join(configDir, "Join popup.ini"), []byte("; 入服公告菜单配置\n; [join_popup] 段使用键值对，其余段落按原样保留多行文本\n; 文本支持 Mindustry 颜色标签和占位符: {server_name} {player_name} {current_map} {players} {link_url}\n; 如果文本里直接换行，游戏内也会按换行显示\n\n[join_popup]\nenabled = 1 ; 1 = 开启，0 = 关闭\ndelay_ms = 300 ; 玩家完成入服同步后延迟多少毫秒再弹出\n\n[title]\n[accent]服务器公告[]\n\n[message]\n欢迎 [green]{player_name}[] 来到 [white]{server_name}[]\n当前地图: [white]{current_map}[]\n请选择下方按钮。\n\n[announcement]\n[accent]服务器公告[]\n\n1. 请遵守服务器规则。\n2. 如有问题可先查看快速帮助。\n3. 需要联系管理时请使用群或外部链接。\n\n[link_url]\nhttps://example.com\n\n[help]\n[accent]帮助为分页弹窗[]\n第 1 页显示基础指令与投票换图；\n第 2 页显示常用与 OP 指令；\n第 3 页显示单位控制。\n"), 0o644)
	...
	_ = writeIfMissing(filepath.Join(configDir, "Vote map.ini"), []byte("; 投票换图配置\n; 控制换图投票持续时间，以及左侧投票状态弹窗刷新频率与位置\n\n[map_vote]\nduration_sec = 15 ; 单次投票持续时间（秒）\nstatus_refresh_ms = 1500 ; 左侧投票状态刷新间隔（毫秒）\npopup_duration_ms = 1800 ; 单次投票状态弹窗持续时间（毫秒），建议略大于刷新间隔\nhome_link_url = https://example.com/votemap ; 换图首页“打开链接”按钮跳转地址，留空则隐藏按钮\nalign = top_left ; 对齐方式：top_left / top / left / center / bottom_right 等\ntop = 220 ; 顶部偏移\nleft = 0 ; 左侧偏移\nbottom = 0 ; 底部偏移\nright = 0 ; 右侧偏移\n"), 0o644)
