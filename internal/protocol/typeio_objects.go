...
// ID returns the building's entity id. In Mindustry a Building's entity id is
// its packed tile position, which is exactly what PosValue holds, so BuildingBox
// satisfies both the Building and Entity interfaces and serializes correctly on
// the wire (WriteEntity writes ID()).
func (b BuildingBox) ID() int32 { return b.PosValue }
