...
func (b testBlock) ContentType() ContentType { return ContentBlock }
func (b testBlock) ID() int16                { return b.id }
...
func TestReadClientPlansHandlesResolvedNonRotatingBlockWithRotateLikeName(t *testing.T) {
	w := NewWriter()
	if err := w.WriteInt16(1); err != nil {
		t.Fatalf("write amount: %v", err)
	}
	if err := w.WriteUint16(21); err != nil {
		t.Fatalf("write x: %v", err)
	}
	if err := w.WriteUint16(34); err != nil {
		t.Fatalf("write y: %v", err)
	}
	if err := w.WriteInt16(77); err != nil {
		t.Fatalf("write block id: %v", err)
	}
	if err := w.WriteByte(0); err != nil {
		t.Fatalf("write nil config: %v", err)
	...
	r := NewReaderWithContext(w.Bytes(), &TypeIOContext{
		BlockLookup: func(id int16) Block {
			if id == 77 {
				return testBlock{id: id, name: "mender"}
			}
			return nil
		},
	})
	...
	if plans[0].X != 21 || plans[0].Y != 34 {
		t.Fatalf("unexpected plan position: (%d,%d)", plans[0].X, plans[0].Y)
	}
	if plans[0].Rotation != 0 {
		t.Fatalf("expected rotation 0 for non-rotating block, got %d", plans[0].Rotation)
	}
	if plans[0].Block == nil || plans[0].Block.ID() != 77 || plans[0].Block.Name() != "mender" {
		t.Fatalf("expected resolved mender block, got %#v", plans[0].Block)
	}
	if plans[0].Config != nil {
		t.Fatalf("expected nil config, got %#v", plans[0].Config)
	}
