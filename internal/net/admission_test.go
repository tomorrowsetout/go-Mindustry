...
// TestAdmissionNameRegexBans verifies the regex name-ban behaviour mirrors the
// Java implementation: case-insensitive substring matching, invalid patterns
// skipped, and exact-name bans still honoured alongside regexes.
func TestAdmissionNameRegexBans(t *testing.T) {
	tests := []struct {
		name    string
		regexes []string
		player  string
		...
		{name: "substring match", regexes: []string{"bot"}, player: "superbot99", banned: true},
		{name: "case-insensitive", regexes: []string{"BOT"}, player: "MyBotHere", banned: true},
		{name: "regex with comma survives", regexes: []string{"^(alpha,beta)$"}, player: "alpha,beta", banned: true},
		{name: "comma regex anchored no partial", regexes: []string{"^(alpha,beta)$"}, player: "alpha", banned: false},
		{name: "anchored regex no match", regexes: []string{"^bot$"}, player: "botmaster", banned: false},
		{name: "anchored regex matches exactly", regexes: []string{"^bot$"}, player: "BOT", banned: true},
		{name: "invalid regex skipped", regexes: []string{"[unclosed", "bot"}, player: "x bot y", banned: true},
		{name: "only invalid regex no ban", regexes: []string{"[unclosed"}, player: "anything", banned: false},
		{name: "no match", regexes: []string{"^spam"}, player: "regular", banned: false},
		{name: "empty player name never banned", regexes: []string{".*"}, player: "   ", banned: false},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			policy := normalizeAdmissionPolicy(AdmissionPolicy{
				BannedNameRegexes: tc.regexes,
			})
			if got := admissionNameBanned(policy, tc.player); got != tc.banned {
				t.Fatalf("admissionNameBanned(%q) = %v, want %v", tc.player, got, tc.banned)
			}
		})
	}
}

// TestAdmissionExactAndRegexBansCoexist ensures the original exact-name ban
// still works when regexes are also configured.
func TestAdmissionExactAndRegexBansCoexist(t *testing.T) {
	policy := normalizeAdmissionPolicy(AdmissionPolicy{
		BannedNames:       []string{"alpha"},
		BannedNameRegexes: []string{"^bot"},
	})
	if !admissionNameBanned(policy, "ALPHA") {
		t.Fatal("exact banned name should still match")
	}
	if !admissionNameBanned(policy, "botnet") {
		t.Fatal("regex ban should match")
	}
	if admissionNameBanned(policy, "gamma") {
		t.Fatal("unrelated name must not be banned")
	}
}
