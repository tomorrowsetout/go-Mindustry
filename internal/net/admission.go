...
type AdmissionPolicy struct {
	...
	ExpectedMods       []string
	BannedNames        []string
	BannedNameRegexes  []string
	BannedSubnets      []string
	RecentKickDuration time.Duration

	// bannedNamePatterns holds the compiled, case-insensitive regexes derived
	// from BannedNameRegexes. It is populated by normalizeAdmissionPolicy so
	// callers never pay compilation cost on the hot connect path. Invalid
	// regexes are dropped silently, matching Java's load() behaviour.
	bannedNamePatterns []*regexp.Regexp
}
...
func normalizeAdmissionPolicy(policy AdmissionPolicy) AdmissionPolicy {
	...
	policy.BannedNameRegexes = normalizeStringSlice(policy.BannedNameRegexes)
	...
	policy.bannedNamePatterns = compileBannedNamePatterns(policy.BannedNameRegexes)
	return policy
}

// compileBannedNamePatterns compiles the name ban regexes into
// case-insensitive matchers. Patterns that fail to compile are skipped, which
// mirrors how the Java implementation silently ignores bad entries while
// loading persisted bans.
func compileBannedNamePatterns(regexes []string) []*regexp.Regexp {
	if len(regexes) == 0 {
		return nil
	}
...
func admissionNameBanned(policy AdmissionPolicy, name string) bool {
	...
	// Regex bans use substring matching (Java: pattern.matcher(name).find())
	// and are compiled case-insensitive, so matching against the lowercased,
	// color-stripped name is equivalent to matching the raw name.
	for _, pattern := range policy.bannedNamePatterns {
