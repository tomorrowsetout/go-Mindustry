...
type Server struct {
	...
	udpConn         *net.UDPConn
	udpWriteFn      func([]byte, *net.UDPAddr) (int, error)
	...
	// --- Vanilla client->server packet hooks (closure of the inbound gap) ---
	// These let game-state / logic modules plug in without touching the
	// dispatch switch in handlePacket. All are nil-safe: if unset, the packet
	// is received and traced but has no server-side effect yet.
	OnTileTap                    func(c *Conn, tilePos int32)
	OnTextInput                  func(c *Conn, id int32, title, message string, numeric, allowEmpty bool)
	OnCopyToClipboard            func(c *Conn, text string)
	OnAdminRequest               func(c *Conn, action protocol.AdminAction, params any)
	OnClientLogicData            func(c *Conn, channel string, value any, reliable bool)
	OnClientPlanSnapshotReceived func(c *Conn, groupID int32, plans any)
	OnDebugStatus                func(c *Conn, value, lastClientSnapshot, snapshotsSent int32)
	OnServerRelay                func(c *Conn, typ, contents string)
	OnServerRelayBinary          func(c *Conn, typ string, contents []byte)
	...
	// ExternalAssetsFn returns the external (workshop/mod) assets the server
	// wants the client to pull before the world stream, for the 159.7 asset
	// handshake. Returning nil/empty means the world has no external assets and
	// the server sends the WorldStream directly (vanilla behavior).
	ExternalAssetsFn func(c *Conn) []ExternalAsset
...
// data (mirrors mindustry.io.DataAsset.byteHash); Data is the raw asset bytes.
...
func NewServer(addr string, build int) *Server {
	if build <= 0 {
		build = defaultMindustryBuild
	}
...
func normalizePingLocationText(text any) string {
	switch v := text.(type) {
	case string:
		return v
	default:
...
func (s *Server) handlePacket(c *Conn, obj any, fromTCP bool) {
	...
	case *protocol.Remote_NetServer_requestAssets:
		s.handleRequestAssets(c, v)
	case *protocol.Remote_NetServer_requestWorld:
		s.handleRequestWorld(c, v)
	...
	case *protocol.Remote_InputHandler_buildingControlSelect_92:
		if s.DevLogger != nil {
			...
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 92, "Remote_InputHandler_buildingControlSelect_92", fmt.Sprintf("build_id=%d", bid))
		}
		if s.OnBuildingControlSelect != nil && v.Build != nil {
			s.OnBuildingControlSelect(c, v.Build.ID())
		}
	case *protocol.Remote_Units_unitSpawn_51:
		// unitSpawn carries the spawned unit's entity id on the wire. The host
		// game logic owns unit lifecycle and registers it through its own world
		// state, so we only surface the id here for tracing.
		if s.DevLogger != nil {
			var id int32
			if v.Container != nil {
				id = v.Container.ID()
			}
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 51, "Remote_Units_unitSpawn_51", fmt.Sprintf("container_id=%d", id))
		}
	...
	case *protocol.Remote_InputHandler_commandBuilding_77:
		// 命令建筑
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 77, "Remote_InputHandler_commandBuilding_77", fmt.Sprintf("command_building(count=%d)", len(v.Buildings)))
		}
		if s.OnCommandBuilding != nil && len(v.Buildings) > 0 {
			...
			s.OnCommandBuilding(c, v.Buildings, target)
		}
	...
	case *protocol.Remote_InputHandler_transferInventory_79:
		// 转移库存
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 79, "Remote_InputHandler_transferInventory_79", "transfer_inventory")
		}
		if s.OnTransferInventory != nil && v.Build != nil {
			s.OnTransferInventory(c, v.Build.ID())
		}
	...
	case *protocol.Remote_InputHandler_requestBuildPayload_82:
		// 请求建筑载荷
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 82, "Remote_InputHandler_requestBuildPayload_82", "request_build_payload")
		}
		if s.OnRequestBuildPayload != nil && v.Build != nil {
			s.OnRequestBuildPayload(c, v.Build.ID())
		}
	...
	case *protocol.Remote_InputHandler_pickedBuildPayload_84:
		// 建筑运载选择
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 84, "Remote_InputHandler_pickedBuildPayload_84", "picked_build_payload")
		}
		if s.OnRequestBuildPayload != nil && v.Build != nil {
			s.OnRequestBuildPayload(c, v.Build.ID())
		}
	...
	case *protocol.Remote_InputHandler_unitEnteredPayload_87:
		// 单位进入运载
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 87, "Remote_InputHandler_unitEnteredPayload_87", "unit_entered_payload")
		}
		if s.OnUnitEnteredPayload != nil && v.Unit != nil && v.Build != nil {
			s.OnUnitEnteredPayload(c, v.Unit.ID(), v.Build.ID())
		}
	...
	case *protocol.Remote_InputHandler_rotateBlock_89:
		if s.DevLogger != nil {
			...
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 89, "Remote_InputHandler_rotateBlock_89", fmt.Sprintf("build_id=%d dir=%v", bid, v.Direction))
		}
		if s.OnRotateBlock != nil && v.Build != nil {
			s.OnRotateBlock(c, v.Build.ID(), v.Direction)
		}
	...
	case *protocol.Remote_InputHandler_tileConfig_90:
		if s.DevLogger != nil {
			...
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 90, "Remote_InputHandler_tileConfig_90", fmt.Sprintf("build_id=%d value=%T", bid, v.Value))
		}
		if s.OnTileConfig != nil && v.Build != nil {
			s.OnTileConfig(c, v.Build.ID(), v.Value)
		}
	...
	case *protocol.Remote_InputHandler_unitBuildingControlSelect_93:
		if s.DevLogger != nil {
			...
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 93, "Remote_InputHandler_unitBuildingControlSelect_93", fmt.Sprintf("unit=%d build_id=%d", unitID, bid))
		}
		if s.OnUnitBuildingControlSelect != nil && v.Unit != nil && v.Build != nil {
			s.OnUnitBuildingControlSelect(c, v.Unit.ID(), v.Build.ID())
		}
	...
	// --- Inbound gap closure: the 14 client->server packets previously
	//     unhandled by handlePacket. Each dispatches to a nil-safe hook so a
	//     future game-state module can consume it without editing this switch.
	case *protocol.Remote_InputHandler_tileTap_91:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 91, "Remote_InputHandler_tileTap_91", "tile_tap")
		}
		if s.OnTileTap != nil && v.Tile != nil {
			s.OnTileTap(c, v.Tile.Pos())
		}
	case *protocol.Remote_Menus_textInput_111:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 111, "Remote_Menus_textInput_111", "text_input")
		}
		if s.OnTextInput != nil {
			s.OnTextInput(c, v.TextInputId, v.Title, v.Message, v.Numeric, v.AllowEmpty)
		}
	case *protocol.Remote_Menus_copyToClipboard_129:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 129, "Remote_Menus_copyToClipboard_129", "copy_to_clipboard")
		}
		if s.OnCopyToClipboard != nil {
			s.OnCopyToClipboard(c, v.Text)
		}
	case *protocol.Remote_NetServer_adminRequest_49:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 49, "Remote_NetServer_adminRequest_49", "admin_request")
		}
		if s.OnAdminRequest != nil {
			s.OnAdminRequest(c, v.Action, v.Params)
		}
	case *protocol.Remote_NetServer_clientLogicDataReliable_43:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 43, "Remote_NetServer_clientLogicDataReliable_43", "client_logic_data_reliable")
		}
		if s.OnClientLogicData != nil {
			s.OnClientLogicData(c, v.Channel, v.Value, true)
		}
	case *protocol.Remote_NetServer_clientLogicDataUnreliable_44:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 44, "Remote_NetServer_clientLogicDataUnreliable_44", "client_logic_data_unreliable")
		}
		if s.OnClientLogicData != nil {
			s.OnClientLogicData(c, v.Channel, v.Value, false)
		}
	case *protocol.Remote_NetServer_clientPlanSnapshotReceived_47:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 47, "Remote_NetServer_clientPlanSnapshotReceived_47", "client_plan_snapshot_received")
		}
		if s.OnClientPlanSnapshotReceived != nil {
			s.OnClientPlanSnapshotReceived(c, v.GroupId, v.Plans)
		}
	case *protocol.Remote_NetServer_debugStatusClient_37:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 37, "Remote_NetServer_debugStatusClient_37", "debug_status_client")
		}
		if s.OnDebugStatus != nil {
			s.OnDebugStatus(c, v.Value, v.LastClientSnapshot, c.snapshotsSent.Load())
		}
	case *protocol.Remote_NetServer_debugStatusClientUnreliable_38:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 38, "Remote_NetServer_debugStatusClientUnreliable_38", "debug_status_client_unreliable")
		}
		if s.OnDebugStatus != nil {
			s.OnDebugStatus(c, v.Value, v.LastClientSnapshot, c.snapshotsSent.Load())
		}
	case *protocol.Remote_NetServer_serverPacketReliable_39:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 39, "Remote_NetServer_serverPacketReliable_39", "server_packet_reliable")
		}
		if s.OnServerRelay != nil {
			s.OnServerRelay(c, v.Type, v.Contents)
		}
	case *protocol.Remote_NetServer_serverPacketUnreliable_40:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 40, "Remote_NetServer_serverPacketUnreliable_40", "server_packet_unreliable")
		}
		if s.OnServerRelay != nil {
			s.OnServerRelay(c, v.Type, v.Contents)
		}
	case *protocol.Remote_NetServer_serverBinaryPacketReliable_41:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 41, "Remote_NetServer_serverBinaryPacketReliable_41", "server_binary_packet_reliable")
		}
		if s.OnServerRelayBinary != nil {
			s.OnServerRelayBinary(c, v.Type, v.Contents)
		}
	case *protocol.Remote_NetServer_serverBinaryPacketUnreliable_42:
		if s.DevLogger != nil {
			s.DevLogger.LogPacketReceived(c.id, c.playerID, 42, "Remote_NetServer_serverBinaryPacketUnreliable_42", "server_binary_packet_unreliable")
		}
		if s.OnServerRelayBinary != nil {
			s.OnServerRelayBinary(c, v.Type, v.Contents)
		}
...
func (s *Server) Broadcast(obj any) {
	...
	// Serialize once and share the frame bytes across every recipient instead
	// of re-encoding the same object per connection.
	encoded, ok := s.preEncode(obj)
	var toSend any = obj
	if ok {
		toSend = encoded
	}
...
func (s *Server) BroadcastUnreliable(obj any) {
	...
	_, isBlockSnapshot := obj.(*protocol.Remote_NetClient_blockSnapshot_34)
	s.mu.Lock()
	peers := make([]*Conn, 0, len(s.conns))
	for c := range s.conns {
		peers = append(peers, c)
	...
	// Serialize once and share the frame bytes across every recipient.
	encoded, ok := s.preEncode(obj)
	var toSend any = obj
	if ok {
		toSend = encoded
	...
	for _, peer := range peers {
		...
		_ = s.sendUnreliable(peer, toSend)
	}
}
...
type Conn struct {
	...
	// 159.7 external-asset handshake state (mirrors mindustry.net.NetConnection).
	determiningAssets     bool
	receivingAssets       bool
...
func (c *Conn) ReadObject() (any, error) {
	for {
		...
		payload := getPooledBuf(n)
		if _, err := io.ReadFull(c.Conn, payload); err != nil {
			putPooledBuf(payload)
			return nil, err
		}
...
func (c *Conn) sendNow(obj any) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.sendNowLocked(obj)
}

func (c *Conn) SendMany(objs ...any) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	for _, obj := range objs {
		...
		if err := c.sendNowLocked(obj); err != nil {
			return err
		}
	}
	return nil
...
func (c *Conn) sendNowLocked(obj any) error {
	if c.serial == nil {
		return errors.New("serializer unavailable")
	}

	// Fast path: reuse a payload pre-encoded once by a broadcast fan-out.
	...
	if c.onSend != nil {
		c.onSend(original, packetID, frameworkID, len(payload))
	}
	c.sendCount.Add(1)
	c.bytesSent.Add(int64(len(payload)))
	c.recordSend(original, int64(len(payload)))
	return nil
}
...
func (c *Conn) Encode(obj any) ([]byte, int, int, error) {
	...
	// Return an owned copy: the pooled buffer goes back to the pool below.
...
func (c *Conn) SendStream(typeID byte, payload []byte) error {
	...
	for len(payload) > 0 {
		// Match Mindustry ArcNetProvider InputStreamSender(stream, 1024).
		chunkLen := 1024
...
func (s *Server) sendUDP(addr *net.UDPAddr, obj any) error {
	...
	_, err := s.writeUDP(buf.Bytes(), addr)
	return err
}
...
func (s *Server) sendUDPRegisterAck(addr *net.UDPAddr, id int32) error {
	...
	_, err := s.writeUDP(buf, addr)
	return err
}

func (s *Server) canWriteUDP() bool {
	return s != nil && (s.udpConn != nil || s.udpWriteFn != nil)
}

func (s *Server) writeUDP(payload []byte, addr *net.UDPAddr) (int, error) {
	if s == nil || addr == nil {
		return 0, errors.New("udp not ready")
	}
	if s.udpWriteFn != nil {
		return s.udpWriteFn(payload, addr)
	}
	if s.udpConn == nil {
		return 0, errors.New("udp not ready")
	}
	return s.udpConn.WriteToUDP(payload, addr)
}
...
// TCPAddr returns the actual bound TCP address once Serve has started. Tests
// and orchestrators use it to discover the ephemeral port chosen for
...
func (s *Server) handleConnectPacket(c *Conn, packet *protocol.ConnectPacket) {
	...
	if err := s.sendWorldAndAssets(c, packet); err != nil {
...
func (s *Server) postConnectLoop(c *Conn) {
	...
	for {
		...
		case now := <-snapshotTicker.C:
			...
			if s.snapshotCycleUsesTCPFallback(c) {
				objs := make([]any, 0, 1+len(packets)+1)
				objs = append(objs, state)
				for _, packet := range packets {
					objs = append(objs, packet)
				}
				if hidden := hiddenSnapshotPacket(hiddenIDs); hidden != nil {
					objs = append(objs, hidden)
				}
				if err := c.SendMany(objs...); err != nil {
					fmt.Printf("[net] snapshot cycle send failed id=%d err=%v\n", c.id, err)
					s.emitEvent(c, "snapshot_cycle_send_failed", "*protocol.Remote_NetClient_stateSnapshot_35", err.Error())
					return
				}
...
func (s *Server) sendHiddenSnapshotToConn(c *Conn, hiddenIDs []int32) error {
	...
	packet := hiddenSnapshotPacket(hiddenIDs)
	if packet == nil {
		return nil
	}
	return s.sendUnreliable(c, packet)
}

func hiddenSnapshotPacket(hiddenIDs []int32) *protocol.Remote_NetClient_hiddenSnapshot_33 {
	if len(hiddenIDs) == 0 {
		return nil
	}
	return &protocol.Remote_NetClient_hiddenSnapshot_33{
		Ids: protocol.IntSeq{Items: append([]int32(nil), hiddenIDs...)},
	}
}

func (s *Server) snapshotCycleUsesTCPFallback(c *Conn) bool {
	if s == nil || c == nil || !s.UdpFallbackTCP {
		return false
	}
	return !s.canWriteUDP() || c.UDPAddr() == nil
}
	...
	if s.canWriteUDP() {
		if addr := c.UDPAddr(); addr != nil {
			...
			for attempt := 0; attempt <= retries; attempt++ {
				_, werr := s.writeUDP(payload, addr)
				if werr == nil {
					if c.onSend != nil {
						c.onSend(original, packetID, frameworkID, len(payload))
					}
...
const (
	maxEntitySnapshotData                  = 32000
	entitySnapshotEntryInitialCapacity     = 128
	entitySnapshotMinPacketInitialCapacity = 256
)

func entitySnapshotPacketInitialCapacity(entries int) int {
	if entries <= 0 {
		return entitySnapshotMinPacketInitialCapacity
	}
	capacity := entries * entitySnapshotEntryInitialCapacity
	if capacity < entitySnapshotMinPacketInitialCapacity {
		return entitySnapshotMinPacketInitialCapacity
	}
	if capacity > maxEntitySnapshotData {
		return maxEntitySnapshotData
	}
	return capacity
}
...
func (s *Server) buildEntitySnapshotBaseFresh() (entitySnapshotBase, error) {
	...
	if s.ExtraEntitySnapshotFn != nil {
		legacyWriter := protocol.NewWriterWithContextCapacity(s.TypeIO, entitySnapshotMinPacketInitialCapacity)
...
func (s *Server) materializeEntitySnapshotPackets(viewer *Conn, base entitySnapshotBase) ([]*protocol.Remote_NetClient_entitySnapshot_32, []int32, error) {
	estimatedEntries := len(base.extraEntries) + len(base.players)*2
	writer := protocol.NewWriterWithContextCapacity(s.TypeIO, entitySnapshotPacketInitialCapacity(estimatedEntries))
...
func (s *Server) encodeEntitySnapshotEntry(entity protocol.UnitSyncEntity) ([]byte, error) {
	...
	ew := protocol.NewWriterWithContextCapacity(s.TypeIO, entitySnapshotEntryInitialCapacity)
...
func (s *Server) BroadcastTileConfig(pos int32, value any, except *Conn) {
	...
	for _, peer := range peers {
		...
		packet := &protocol.Remote_InputHandler_tileConfig_90{
			Build: &protocol.EntityBox{IDValue: pos},
			Value: clonedValue,
		}
		_ = peer.SendAsync(packet)
	}
...
func (s *Server) serializeEntity(entity protocol.UnitSyncEntity) []byte {
	...
	w := protocol.NewWriterWithContextCapacity(s.TypeIO, entitySnapshotEntryInitialCapacity)
...
// sendWorldAndAssets mirrors mindustry.core.NetServer.sendWorldAndAssets:
// if the world references external assets, the server first streams an
// AssetRequirementStream and lets the client drive the asset pull
// (requestAssets -> AssetStream -> requestWorld -> WorldStream). Otherwise it
// sends the WorldStream directly (vanilla 158-compatible path).
func (s *Server) sendWorldAndAssets(c *Conn, pkt *protocol.ConnectPacket) error {
	assets := s.externalAssetsFor(c)
	if len(assets) == 0 {
		return s.sendWorldHandshake(c, pkt)
	}
	c.determiningAssets = true
	c.receivingAssets = false
	return s.sendAssetRequirements(c, assets)
}

func (s *Server) externalAssetsFor(c *Conn) []ExternalAsset {
	if s == nil || s.ExternalAssetsFn == nil {
		return nil
	}
	return s.ExternalAssetsFn(c)
}

// sendAssetRequirements streams the AssetRequirementStream (base packet id 4)
// carrying the list of required asset hashes, then waits for the client to
// drive requestAssets/requestWorld.
func (s *Server) sendAssetRequirements(c *Conn, assets []ExternalAsset) error {
	body := writeRequiredAssetsBody(assets)
	reqID, ok := s.Registry.PacketID(&protocol.AssetRequirementStream{})
	if !ok {
		return fmt.Errorf("asset requirement stream packet id not found")
	}
	return c.SendStream(reqID, body)
}

// handleRequestAssets (id 52) mirrors NetServer.requestAssets: stream the
// requested AssetStream (base packet id 5) payloads, then (when all assets are
// received) the client sends requestWorld which triggers the WorldStream.
func (s *Server) handleRequestAssets(c *Conn, p *protocol.Remote_NetServer_requestAssets) {
	if c == nil || !c.hasBegunConnecting || !c.determiningAssets || c.receivingAssets || c.hasConnected {
		return
	}
	c.determiningAssets = false
	c.receivingAssets = true

	assets := s.externalAssetsFor(c)
	// No asset ids requested (client already has everything cached): send the
	// world immediately.
	if len(p.Ids) == 0 {
		c.receivingAssets = false
		_ = s.sendWorldHandshake(c, nil)
		return
	}

	requested := make([]ExternalAsset, 0, len(p.Ids))
	for _, id := range p.Ids {
		if int(id) >= len(assets) || id < 0 {
			continue
		}
		requested = append(requested, assets[id])
	}
	if len(requested) == 0 {
		c.receivingAssets = false
		_ = s.sendWorldHandshake(c, nil)
	...
	body := writeAssetsBody(requested)
	assetID, ok := s.Registry.PacketID(&protocol.AssetStream{})
	if !ok {
		c.receivingAssets = false
		_ = s.sendWorldHandshake(c, nil)
		return
	}
	if err := c.SendStream(assetID, body); err != nil {
		fmt.Printf("[net] asset stream send failed id=%d err=%v\n", c.id, err)
		c.receivingAssets = false
	}
}

// handleRequestWorld (id 55) mirrors NetServer.requestWorld: the client finished
// pulling assets and is ready for the world stream.
func (s *Server) handleRequestWorld(c *Conn, _ *protocol.Remote_NetServer_requestWorld) {
	if c == nil || !c.hasBegunConnecting || c.determiningAssets || !c.receivingAssets || c.hasConnected {
		return
	}
	c.receivingAssets = false
...
// writeRequiredAssetsBody mirrors NetworkIO.writeRequiredAssets: int32 count
// followed by each asset's 32-byte hash.
func writeRequiredAssetsBody(assets []ExternalAsset) []byte {
	w := protocol.NewWriter()
	_ = w.WriteInt32(int32(len(assets)))
	for _, a := range assets {
		hash := a.Hash
		if len(hash) < 32 {
			hash = append(hash, make([]byte, 32-len(hash))...)
		}
		_ = w.WriteBytes(hash[:32])
	}
	return w.Bytes()
}

// writeAssetsBody mirrors NetworkIO.writeAssets: int32 count followed by each
// asset's int32 length + bytes.
func writeAssetsBody(assets []ExternalAsset) []byte {
	w := protocol.NewWriter()
	_ = w.WriteInt32(int32(len(assets)))
	for _, a := range assets {
		_ = w.WriteInt32(int32(len(a.Data)))
		_ = w.WriteBytes(a.Data)
	}
	return w.Bytes()
}
