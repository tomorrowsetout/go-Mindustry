...
// officialPacketID uses the canonical Mindustry build 158 packet registration order.
...
func (s *Serializer) ReadObject(buf *bytes.Reader) (any, error) {
	...
	payload := getPooledBuf(int(length))
	defer putPooledBuf(payload)
	if comp == 0 {
		...
		compressed := getPooledBuf(buf.Len())
		if _, err := io.ReadFull(buf, compressed); err != nil {
			putPooledBuf(compressed)
			return nil, err
		}
		_, uerr := lz4.UncompressBlock(compressed, payload)
		putPooledBuf(compressed)
		if uerr != nil {
			return nil, uerr
		}
