...
func (w *World) updateEntityStatuses(e *RawEntity, dt float32) {
	...
	for _, st := range e.Statuses {
		...
		activeIdx := len(active) - 1
		e.StatusDamageMul *= prof.DamageMultiplier
		e.StatusHealthMul *= prof.HealthMultiplier
		e.StatusSpeedMul *= prof.SpeedMultiplier
		e.StatusReloadMul *= prof.ReloadMultiplier
		e.StatusBuildSpeedMul *= prof.BuildSpeedMultiplier
		e.StatusDragMul *= prof.DragMultiplier
		...
		if prof.IntervalDamageTime > 0 {
			active[activeIdx].DamageTime += dt
			for active[activeIdx].DamageTime >= prof.IntervalDamageTime {
				active[activeIdx].DamageTime -= prof.IntervalDamageTime
...
func entitySpeedMultiplier(e RawEntity) float32 {
	speedMul := float32(1)
	if e.StatusSpeedMul > 0 || (e.StatusSpeedMul == 0 && len(e.Statuses) > 0) {
