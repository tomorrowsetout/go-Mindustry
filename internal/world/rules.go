...
const spawnGroupNever = int32(1<<31 - 1)
...
type SpawnGroupRule struct {
	Type          string   `json:"type"`
	End           int32    `json:"end"`
	Begin         int32    `json:"begin"`
	Spacing       int32    `json:"spacing"`
	Max           int32    `json:"max"`
	UnitScaling   float32  `json:"scaling"`
	Shields       float32  `json:"shields"`
	ShieldScaling float32  `json:"shieldScaling"`
	UnitAmount    int32    `json:"amount"`
...
func (g *SpawnGroupRule) UnmarshalJSON(data []byte) error {
	...
	out := SpawnGroupRule{
		...
		Spacing:     1,
		Max:         40,
		UnitScaling: float32(spawnGroupNever),
		UnitAmount:  1,
		Spawn:       -1,
		Effect:      aux.Effect,
		Payloads:    append([]string(nil), aux.Payloads...),
	}
	if aux.Begin != nil {
		out.Begin = *aux.Begin
	}
	...
	if aux.Spacing != nil {
		out.Spacing = *aux.Spacing
	}
	if aux.Max != nil {
		out.Max = *aux.Max
	}
	if aux.UnitScaling != nil {
		out.UnitScaling = *aux.UnitScaling
	}
	if aux.Shields != nil {
		out.Shields = *aux.Shields
	}
	if aux.ShieldScaling != nil {
		out.ShieldScaling = *aux.ShieldScaling
	}
	if aux.UnitAmount != nil {
		out.UnitAmount = *aux.UnitAmount
	}
	if aux.Spawn != nil {
		out.Spawn = *aux.Spawn
	}
	switch team := aux.Team.(type) {
	case string:
		out.Team = team
	case float64:
		out.Team = strconv.Itoa(int(team))
	}
	*g = out
	return nil
}

func (g SpawnGroupRule) Spawned(wave int32) int32 {
	spacing := g.Spacing
	if spacing == 0 {
		spacing = 1
	}
	if wave < g.Begin || wave > g.End || (wave-g.Begin)%spacing != 0 {
		return 0
	}
	scaling := g.UnitScaling
	scaled := int32(0)
	if scaling > 0 && scaling < float32(spawnGroupNever) {
		scaled = int32(float32((wave-g.Begin)/spacing) / scaling)
	}
	amount := g.UnitAmount + scaled
	if amount > g.Max {
		amount = g.Max
	}
	if amount < 0 {
...
func (g SpawnGroupRule) Shield(wave int32) float32 {
	shield := g.Shields + g.ShieldScaling*float32(wave-g.Begin)
	if shield < 0 {
		return 0
	}
	return shield
...
type Rules struct {
	...
	// 其他字段
	Spawns          []SpawnGroupRule `json:"spawns"`
...
func (rm *RulesManager) Clone() *Rules {
	...
	out := *rm.rules
	if len(rm.rules.Teams) > 0 {
		out.Teams = make(map[string]TeamRule, len(rm.rules.Teams))
		for k, v := range rm.rules.Teams {
			out.Teams[k] = v
		}
	}
	if len(rm.rules.Spawns) > 0 {
		out.Spawns = append([]SpawnGroupRule(nil), rm.rules.Spawns...)
	}
	return &out
}
