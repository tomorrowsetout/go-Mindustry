...
func (w *World) stepBurstDrillProduction(delta time.Duration) time.Duration {
	...
	dispatch := w.parallelizeRanges(len(w.burstDrillTilePositions), func(_ int, start, end int) {
		for i := start; i < end; i++ {
			...
			item, count, _, oreOK := w.countDrillOreFilteredLocked(tile, prof.Tier, prof.BlockedItems)
			samples[i] = burstDrillOreSample{item: item, count: count, ok: oreOK, prof: prof, hasProf: true}
		}
	})
	for i, pos := range w.burstDrillTilePositions {
		...
		w.stepSingleBurstDrillLocked(pos, tile, sample.prof, &state, *sample, deltaFrames, deltaSeconds)
