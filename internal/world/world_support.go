...
func (w *World) stepMendProjectorLocked(pos int32, tile *Tile, prof mendProjectorProfile, state *mendProjectorState, dt, deltaFrames float32) {
	...
	optionalEfficiency := supportPhaseOptionalEfficiency(tile, efficiency)
	state.PhaseHeat = lerpDeltaf(state.PhaseHeat, optionalEfficiency, 0.1, deltaFrames)
	if optionalEfficiency > 0 && canHeal {
		state.UseProgress += deltaFrames
		if state.UseProgress >= prof.UseTime {
			consumeSupportPhaseItemLocked(w, pos, tile)
			for state.UseProgress >= prof.UseTime {
				state.UseProgress -= prof.UseTime
			}
		}
	}
...
func (w *World) stepOverdriveProjectorLocked(pos int32, tile *Tile, prof overdriveProjectorProfile, state *overdriveProjectorState, dt, deltaFrames float32) {
	...
	if efficiency > 0 && !hasRequiredItemsLocked(tile.Build, overdriveProjectorRequiredItems(prof)) {
		efficiency = 0
	}
	...
	if prof.HasBoost {
		state.PhaseHeat = lerpDeltaf(state.PhaseHeat, overdriveProjectorOptionalEfficiency(tile, efficiency), 0.1, deltaFrames)
	} else {
		state.PhaseHeat = 0
	}
	...
	if state.UseProgress >= prof.UseTime {
		w.consumeOverdriveProjectorItemsLocked(pos, tile, prof)
		for state.UseProgress >= prof.UseTime {
			state.UseProgress -= prof.UseTime
		}
	}
...
func overdriveProjectorRequiredItems(prof overdriveProjectorProfile) []ItemStack {
	if prof.HasBoost {
		return nil
	}
	return []ItemStack{
		{Item: phaseFabricItemID, Amount: 1},
		{Item: siliconItemID, Amount: 1},
	}
}

func overdriveProjectorOptionalEfficiency(tile *Tile, efficiency float32) float32 {
	if tile == nil || tile.Build == nil || efficiency <= 0 {
		return 0
	}
	if tile.Build.ItemAmount(phaseFabricItemID) <= 0 {
		return 0
	}
	return 1
}

func supportPhaseOptionalEfficiency(tile *Tile, efficiency float32) float32 {
	if tile == nil || tile.Build == nil || efficiency <= 0 {
		return 0
	}
	if tile.Build.ItemAmount(phaseFabricItemID) <= 0 {
...
func consumeSupportPhaseItemLocked(w *World, pos int32, tile *Tile) {
	if tile == nil || tile.Build == nil || tile.Build.ItemAmount(phaseFabricItemID) <= 0 {
		return
	}
	_ = tile.Build.RemoveItem(phaseFabricItemID, 1)
	if w != nil {
		w.emitBlockItemSyncLocked(pos)
	}
}

func (w *World) consumeOverdriveProjectorItemsLocked(pos int32, tile *Tile, prof overdriveProjectorProfile) {
	...
	required := overdriveProjectorRequiredItems(prof)
	if !hasRequiredItemsLocked(tile.Build, required) {
		return
	}
	removeItemStacksLocked(tile.Build, required)
	w.emitBlockItemSyncLocked(pos)
}
...
func (w *World) stepForceProjectorLocked(pos int32, tile *Tile, prof forceProjectorProfile, state *forceProjectorState, dt, deltaFrames float32) {
	...
	phaseValid := tile.Build.ItemAmount(phaseFabricItemID) > 0
	state.PhaseHeat = lerpDeltaf(state.PhaseHeat, condf(phaseValid, 1, 0), 0.1, deltaFrames)
	if phaseValid {
		state.PhaseUseProgress += deltaFrames
		if state.PhaseUseProgress >= prof.PhaseUseTime {
			consumeSupportPhaseItemLocked(w, pos, tile)
			for state.PhaseUseProgress >= prof.PhaseUseTime {
				state.PhaseUseProgress -= prof.PhaseUseTime
			}
		}
	}
