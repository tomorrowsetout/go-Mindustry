...
func (w *World) blockLookupNameByID(blockID int16) string {
	if blockID <= 0 {
		return ""
	}
	if idx := int(blockID); idx < len(w.blockLookupNamesByIndex) {
		if name := w.blockLookupNamesByIndex[idx]; name != "" {
			return name
		}
	}
	if len(w.blockLookupNamesByID) > 0 {
		if name := w.blockLookupNamesByID[blockID]; name != "" {
			return name
		}
	}
	return normalizeBlockLookupName(w.blockNameByID(blockID))
}

func (w *World) unitLookupNameByID(typeID int16) string {
	if typeID <= 0 {
		return ""
	}
	if idx := int(typeID); idx < len(w.unitLookupNamesByIndex) {
		if name := w.unitLookupNamesByIndex[idx]; name != "" {
			return name
		}
	}
	if len(w.unitLookupNamesByID) > 0 {
		if name := w.unitLookupNamesByID[typeID]; name != "" {
			return name
		}
	}
	name := ""
...
func (w *World) teamCoreItemAmountLocked(team TeamID, item ItemID) int32 {
	var total int32
	for _, core := range w.teamCoreBuildsLocked(team) {
...
func (w *World) emitTeamCoreItemsLocked(team TeamID, items []ItemID) {
	...
	if len(items) == 1 {
		...
		w.entityEvents = append(w.entityEvents, EntityEvent{
			...
			BuildTeam:  team,
			ItemID:     item,
			ItemAmount: w.teamCoreItemAmountLocked(team, item),
		})
		return
	}
