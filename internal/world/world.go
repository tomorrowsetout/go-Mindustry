...
type Config struct {
	...
	PerfLogsEnabled        bool
	PerfLogWriter          io.Writer
}
...
type teamBuildingSpatialEntry struct {
	Team TeamID
	Idx  *buildingSpatialIndex
}

type unitAbilityCandidate struct {
	Index  int
	Prof   unitRuntimeProfile
	Radius float32
}
...
type World struct {
	...
	blockLookupNamesByID        map[int16]string
	...
	unitLookupNamesByIndex      []string
	unitTypeDefsByID            map[int16]vanilla.UnitTypeDef
	buildStates                 map[int32]buildCombatState
	tickStartTurretCoolants     map[int32]map[LiquidID]float32
	tickLiquidReceipts          map[int32]map[LiquidID][]liquidTickReceipt
	...
	unitAbilityCandidates       []unitAbilityCandidate
	...
	teamBuildingSpatialEntries  []teamBuildingSpatialEntry
...
func initialBuildCombatStateFromTile(tile *Tile, prof buildingWeaponProfile) buildCombatState {
	...
	if tile != nil {
		state.TurretRotation = float32(tile.Rotation) * 90
	}
	if reload, rotation, ok := decodeTurretMapSyncTail(tile); ok {
		state.TurretRotation = normalizeAngleDeg(rotation)
		if prof.Interval > 0 {
			state.Cooldown = clampf(prof.Interval-reload, 0, prof.Interval)
		}
	}
	return state
}

func decodeTurretMapSyncTail(tile *Tile) (float32, float32, bool) {
	if tile == nil || tile.Build == nil || tile.Build.MapSyncRevision < 1 || len(tile.Build.MapSyncTail) < 8 {
		return 0, 0, false
	}
	r := protocol.NewReader(tile.Build.MapSyncTail)
	...
	if math.IsNaN(float64(reload)) || math.IsInf(float64(reload), 0) || math.IsNaN(float64(rotation)) || math.IsInf(float64(rotation), 0) {
		return 0, 0, false
	}
...
var turretLiquidCapacityByName = map[string]float32{
	"wave":       10,
	"tsunami":    40,
	"foreshadow": 60,
	"spectre":    120,
...
var turretCoolantAmountByName = map[string]float32{
	"duo":        0.1,
	"scatter":    0.2,
	"scorch":     0.1,
	"hail":       0.1,
...
var turretCoolantMultiplierByName = map[string]float32{
	"duo":        10.0,
	"scorch":     1.5,
	"hail":       10.0,
	"foreshadow": 0.4,
...
var supportBoostItemsByName = map[string][]ItemID{
	"mender":              {siliconItemID},
	"mend-projector":      {phaseFabricItemID},
	"overdrive-projector": {phaseFabricItemID},
	"overdrive-dome":      {phaseFabricItemID, siliconItemID},
	"force-projector":     {phaseFabricItemID},
	"regen-projector":     {phaseFabricItemID},
}
...
func New(cfg Config) *World {
	...
	perfWriter := cfg.PerfLogWriter
	if perfWriter == nil {
		perfWriter = os.Stderr
	}
...
func (w *World) Step(delta time.Duration) {
	...
	defer func() {
		...
		w.buildingProfileCacheStep = false
	}()
	stepStartedAt := time.Now()
	profileStep := w.perfLogs
	w.tick++
	w.rebuildBuildingProfileBlockCacheLocked()
	w.buildingProfileCacheStep = true
	now := time.Now()
	...
	if dt > 0 {
		...
		if wavesEnabled {
			if !w.waveTimeInitialized {
				w.waveTime = w.nextWaveSpacingSec()
				w.waveTimeInitialized = true
			}
			if waveTimer && w.waveTime > 0 && !w.isWaitingWaveLocked() {
				w.waveTime -= dt
				if w.waveTime < 0 {
					w.waveTime = 0
				}
			...
			if w.waveTime <= 0 {
				...
				w.waveTime = w.waveSpacingSec()
				w.waveTimeInitialized = true
			}
		}
	}

	w.snapshotTickStartTurretCoolantsLocked()

	w.stepFillItemsLocked()

	var pendingBuildStartedAt time.Time
	if profileStep {
		pendingBuildStartedAt = time.Now()
	}
	w.stepPendingBuilds(delta)
	var pendingBuildDur time.Duration
	if profileStep {
		pendingBuildDur = time.Since(pendingBuildStartedAt)
	}

	var pendingBreakStartedAt time.Time
	if profileStep {
		pendingBreakStartedAt = time.Now()
	}
	w.stepPendingBreaks(delta)
	var pendingBreakDur time.Duration
	if profileStep {
	...
	var factoryDur time.Duration
	if profileStep {
		factoryDur = time.Since(factoryStartedAt)
	}

	var itemStartedAt time.Time
	if profileStep {
		itemStartedAt = time.Now()
	}
	itemPerf := w.stepItemLogistics(delta, profileStep && w.shouldProfileItemPerfLocked(now))
	var itemDur time.Duration
	if profileStep {
		itemDur = time.Since(itemStartedAt)
	}

	var payloadStartedAt time.Time
	if profileStep {
		payloadStartedAt = time.Now()
	}
	w.stepPayloadLogistics(delta)
	var payloadDur time.Duration
	if profileStep {
...
func (w *World) clearConfiguredStateLocked(pos int32) {
	...
	delete(w.itemSourceCfg, pos)
	if pos >= 0 && int(pos) < len(w.itemSourceAccumByPos) {
		w.itemSourceAccumByPos[pos] = 0
	}
	delete(w.liquidSourceCfg, pos)
...
func (w *World) stepSandboxSources(delta time.Duration) {
	...
	for _, pos := range w.sandboxItemSourceTiles {
		...
		if item, ok := w.itemSourceCfg[pos]; ok {
			w.pushItemSourceLocked(pos, tile, item, frameDelta*w.buildingTimeScaleLocked(pos))
		}
	}
...
func (w *World) pushItemSourceLocked(pos int32, tile *Tile, item ItemID, frameDelta float32) {
	...
	if pos < 0 || int(pos) >= len(w.itemSourceAccumByPos) {
		...
		for w.itemSourceAccum[pos] >= limit {
			w.dumpVirtualItemLocked(pos, item, nil)
			w.itemSourceAccum[pos] -= limit
		}
		return
	}
	acc := w.itemSourceAccumByPos[pos] + frameDelta
	for acc >= limit {
		w.dumpVirtualItemLocked(pos, item, nil)
		acc -= limit
	}
	w.itemSourceAccumByPos[pos] = acc
}
...
func (w *World) itemCapacityForBlockLocked(tile *Tile) int32 {
	...
	case "thorium-reactor":
		return 30
	case "mender", "mend-projector", "overdrive-projector", "overdrive-dome", "force-projector", "regen-projector":
		return 10
	default:
...
func (w *World) liquidCapacityForBlockLocked(tile *Tile) float32 {
	...
	default:
		...
		if capacity, ok := turretLiquidCapacityByName[name]; ok {
			return capacity
		}
...
func (w *World) stepLiquidBridgeLocked(pos int32, tile *Tile, dt float32) {
	...
	if !linked {
		_ = w.dumpLiquidProportionalLocked(pos, tile, liquid, 1)
		return
	}
	moved := w.moveLiquidProportionalLocked(pos, target, liquid, 1, 1, 0)
	if moved > 0 {
...
func (w *World) stepDirectionalLiquidBridgeLocked(pos int32, tile *Tile, dt float32) {
	...
	if nextPos, ok := w.forwardPosLocked(pos, tile.Rotation); ok {
		if moved := w.moveLiquidProportionalLocked(pos, nextPos, liquid, 1, 1, 0); moved > 0 {
			_ = tile.Build.RemoveLiquid(liquid, moved)
		}
	}
}
...
func (w *World) stepConduitLocked(pos int32, tile *Tile, pressure float32, leaks bool, dt float32) {
	...
	if nextPos, ok := w.forwardPosLocked(pos, tile.Rotation); ok {
		if moved := w.moveLiquidProportionalLocked(pos, nextPos, liquid, pressure, 1, 0); moved > 0 {
			_ = tile.Build.RemoveLiquid(liquid, moved)
			return
		}
	}
...
func blockNeverAcceptsInsertedItemName(name string) bool {
	switch name {
	case "item-source", "liquid-source", "power-source", "battery", "mechanical-pump":
		return true
	default:
		return false
	}
...
func (w *World) blockNeverAcceptsInsertedItemLocked(blockID int16) bool {
	if w == nil || blockID <= 0 {
		return false
	}
	if idx := int(blockID); idx < len(w.itemInsertNeverByBlock) {
		return w.itemInsertNeverByBlock[idx] != 0
	}
	return blockNeverAcceptsInsertedItemName(w.blockNameByID(blockID))
}
...
func firstBuildingLiquid(build *Building) (LiquidID, float32, bool) {
	...
	if liquid, amount, ok := currentBuildingLiquid(build); ok && amount > 0 {
		return liquid, amount, true
	}
	return 0, 0, false
}

func currentBuildingLiquid(build *Building) (LiquidID, float32, bool) {
	if build == nil {
		return 0, 0, false
	}
	if build.CurrentLiquidSet {
		return build.CurrentLiquid, build.LiquidAmount(build.CurrentLiquid), true
	}
	var (
		bestLiquid LiquidID
		bestAmount float32
...
func normalizeBuildingLiquidCurrent(build *Building) {
	if build == nil || build.CurrentLiquidSet {
		return
	}
	if liquid, _, ok := currentBuildingLiquid(build); ok {
		build.CurrentLiquid = liquid
		build.CurrentLiquidSet = true
	}
}
	...
	for team, cores := range w.teamCoreTiles {
		...
		if primary >= 0 && int(primary) < len(w.model.Tiles) {
			if build := w.model.Tiles[primary].Build; build != nil {
				build.Items = cloneItemStacks(normalized)
			}
		}
		for _, corePos := range cores {
...
func (w *World) syncSharedCoreInventoryItemsLocked(team TeamID, proxyPos int32) {
	if w == nil || w.model == nil || proxyPos < 0 || int(proxyPos) >= len(w.model.Tiles) {
		return
	}
	proxy := &w.model.Tiles[proxyPos]
	if proxy.Build == nil {
		return
	}
	items := cloneItemStacks(proxy.Build.Items)
	for _, corePos := range w.teamCoreTiles[team] {
		if corePos < 0 || int(corePos) >= len(w.model.Tiles) {
			continue
		}
		if build := w.model.Tiles[corePos].Build; build != nil {
			build.Items = cloneItemStacks(items)
		}
	}
	for storagePos, linkedCore := range w.storageLinkedCore {
		if linkedCore != proxyPos || storagePos < 0 || int(storagePos) >= len(w.model.Tiles) {
			continue
		}
		if build := w.model.Tiles[storagePos].Build; build != nil {
			build.Items = cloneItemStacks(items)
...
func (w *World) addItemAtLocked(pos int32, item ItemID, amount int32) bool {
	...
	if team, proxyPos, build, ok := w.sharedCoreInventoryLocked(pos); ok {
		build.AddItem(item, amount)
		w.syncSharedCoreInventoryItemsLocked(team, proxyPos)
...
func (w *World) dumpProximityLocked(pos int32) []int32 {
	...
	if !ok {
		...
		if dumpProximityProxySeed != 0 {
			sort.SliceStable(neighbors, func(i, j int) bool {
				ki := dumpProximityProxyKey(neighbors[i])
				kj := dumpProximityProxyKey(neighbors[j])
				if ki != kj {
					return ki < kj
				}
				return false
...
const defaultDumpProximityProxySeed = uint32(799)
const defaultLiquidDumpProximityProxySeed = uint32(14)

var dumpProximityProxySeed = loadDumpProximityProxySeed()
var liquidDumpProximityProxySeed = loadLiquidDumpProximityProxySeed()

func loadDumpProximityProxySeed() uint32 {
	raw := strings.TrimSpace(os.Getenv("MDT_DUMP_PROXIMITY_PROXY_SEED"))
	if raw == "" {
		return defaultDumpProximityProxySeed
	}
...
func dumpProximityProxyKey(pos int32) uint32 {
	return mixDumpProximityProxyKey(uint32(pos)^dumpProximityProxySeed) & 63
}

func loadLiquidDumpProximityProxySeed() uint32 {
	raw := strings.TrimSpace(os.Getenv("MDT_LIQUID_DUMP_PROXIMITY_PROXY_SEED"))
	if raw == "" {
		return defaultLiquidDumpProximityProxySeed
	}
	seed, err := strconv.ParseUint(raw, 10, 32)
	if err != nil {
		return defaultLiquidDumpProximityProxySeed
...
func dumpProximityProxyKeyForSeed(pos int32, seed uint32) uint32 {
	return mixDumpProximityProxyKey(uint32(pos)^seed) & 63
}

func sortedDumpNeighborsForSeed(neighbors []int32, seed uint32) []int32 {
	if len(neighbors) <= 1 {
		return neighbors
	}
	out := append([]int32(nil), neighbors...)
	sort.SliceStable(out, func(i, j int) bool {
		ki := dumpProximityProxyKeyForSeed(out[i], seed)
		kj := dumpProximityProxyKeyForSeed(out[j], seed)
		if ki != kj {
			return ki < kj
		}
		return false
...
func mixDumpProximityProxyKey(x uint32) uint32 {
	x ^= x >> 16
	x *= 0x7feb352d
	x ^= x >> 15
	x *= 0x846ca68b
...
func (w *World) dumpVirtualItemLocked(pos int32, item ItemID, canDump func(int32, ItemID) bool) bool {
	if w.model == nil {
		return false
	}
	neighbors := w.dumpProximityLocked(pos)
	...
	for i := 0; i < len(neighbors); i++ {
		index := (start + i) % len(neighbors)
		target := neighbors[index]
		if target >= 0 && int(target) < len(w.model.Tiles) && w.blockNeverAcceptsInsertedItemLocked(int16(w.model.Tiles[target].Block)) {
			w.advanceDumpIndexLocked(pos, index+1, len(neighbors))
			continue
		}
		if canDump != nil && !canDump(target, item) {
...
func supportBoostAcceptsItem(name string, item ItemID) bool {
	for _, accepted := range supportBoostItemsByName[name] {
		if accepted == item {
			return true
		}
	}
...
func (w *World) maximumAcceptedItemForBlockNameLocked(pos int32, tile *Tile, name string, item ItemID) int32 {
	...
	case "neoplasia-reactor":
		...
		return 0
	case "mender", "mend-projector", "overdrive-projector", "overdrive-dome", "force-projector", "regen-projector":
		if supportBoostAcceptsItem(name, item) {
			return w.itemCapacityForBlockLocked(tile)
		}
		return 0
	}
...
func (w *World) storeAcceptedBuildingItemLocked(pos int32, tile *Tile, item ItemID, amount int32) bool {
	...
	if tile != nil && tile.Build != nil {
		...
		if prof, ok := w.getBuildingWeaponProfile(blockID); ok {
			name = w.blockNameByID(blockID)
			if classifyTurretBlockSyncKind(name, prof) == blockSyncItemTurret {
...
func blockSizeByName(name string) int {
	switch name {
	case "distributor",
		"copper-wall-large", "titanium-wall-large", "plastanium-wall-large", "thorium-wall-large", "phase-wall-large", "surge-wall-large",
		"door-large", "scrap-wall-large", "beryllium-wall-large", "tungsten-wall-large", "blast-door", "reinforced-surge-wall-large", "carbide-wall-large":
		return 2
	case "mechanical-drill", "pneumatic-drill", "rotary-pump", "water-extractor", "graphite-press", "silicon-smelter", "kiln", "plastanium-compressor", "phase-weaver", "cryofluid-mixer", "pyratite-mixer", "blast-mixer", "separator", "coal-centrifuge", "spore-press", "cultivator", "electric-heater", "phase-heater":
		return 2
	case "repair-turret", "unit-repair-tower", "plasma-bore", "mend-projector", "overdrive-projector":
		return 2
	...
	case "container", "reinforced-container":
		return 2
	case "laser-drill", "impulse-pump", "oil-extractor", "melter", "silicon-crucible", "disassembler", "silicon-arc-furnace", "electrolyzer", "atmospheric-concentrator", "oxidation-chamber", "slag-heater", "vent-condenser", "slag-centrifuge", "heat-reactor", "turbine-condenser", "chemical-combustion-chamber", "pyrolysis-generator", "carbide-crucible", "surge-crucible", "cyanogen-synthesizer", "phase-synthesizer", "heat-redirector", "heat-router", "large-plasma-bore", "overdrive-dome", "force-projector":
		return 3
	case "ripple", "cyclone":
		return 3
	case "battery-large", "solar-panel-large", "differential-generator", "beam-tower", "beam-link", "scrap-wall-huge":
		return 3
	case "foreshadow", "spectre", "meltdown", "scrap-wall-gigantic", "thruster":
		return 4
...
func (w *World) blockOccupancySizeForTileLocked(tile *Tile) int {
	size := w.blockSizeForTileLocked(tile)
	if size > 1 {
		return size
	}
	if inferred := w.inferBlockFootprintSizeLocked(tile); inferred > size {
		return inferred
	}
	return size
}

func (w *World) inferBlockFootprintSizeLocked(tile *Tile) int {
	if w == nil || w.model == nil || tile == nil || tile.Block == 0 || tile.Build == nil {
		return 1
	}
	best := 1
	for size := 2; size <= 16; size++ {
		...
		ok := true
		for y := tile.Y + low; ok && y <= tile.Y+high; y++ {
			for x := tile.X + low; x <= tile.X+high; x++ {
				if !w.model.InBounds(x, y) {
					ok = false
					break
				}
				other := &w.model.Tiles[y*w.model.Width+x]
				if other.Block != tile.Block {
					ok = false
					break
				}
...
func (w *World) rebuildBlockOccupancyLocked() {
	...
	w.teamBuildingSpatialEntries = w.teamBuildingSpatialEntries[:0]
...
func (w *World) indexActiveTileLocked(pos int32, tile *Tile) {
	...
	if team != 0 {
		...
		w.teamBuildingSpatialIndexLocked(team).insert(tile.X, tile.Y, pos)
		if isCoreBlockName(name) {
...
func (w *World) setBuildingOccupancyLocked(pos int32, tile *Tile, occupy bool) {
	...
	low, high := blockFootprintRange(w.blockOccupancySizeForTileLocked(tile))
...
func (w *World) waveSpacingSec() float32 {
	rules := w.rulesMgr.Get()
	if rules != nil && rules.WaveSpacing > 0 {
		return rules.WaveSpacing
	}
	return 120
}

func (w *World) isWaitingWaveLocked() bool {
	if w == nil || w.model == nil || w.rulesMgr == nil {
		return false
	}
	rules := w.rulesMgr.Get()
	if rules == nil {
	...
	if !rules.WaitEnemies && !(rules.WinWave > 0 && w.wave >= rules.WinWave) {
		return false
	}
	_, waveTeam := w.teamsFromRulesLocked()
	for _, e := range w.model.Entities {
		if e.Team == waveTeam && e.Health > 0 {
			return true
		}
	}
	return false
...
func (w *World) restoreRuntimeStateFromModelTagsLocked(m *WorldModel) {
	if w == nil || m == nil || len(m.Tags) == 0 {
		return
	}
	if raw := strings.TrimSpace(tagValue(m.Tags, "wave")); raw != "" {
		if parsed, err := strconv.ParseInt(raw, 10, 32); err == nil && parsed > 0 {
			w.wave = int32(parsed)
		}
	}
	if raw := strings.TrimSpace(tagValue(m.Tags, "wavetime", "waveTime")); raw != "" {
		if parsed, err := strconv.ParseFloat(raw, 32); err == nil {
			w.waveTime = float32(parsed) / 60
			if w.waveTime < 0 {
				w.waveTime = 0
			}
			w.waveTimeInitialized = true
	...
	if raw := strings.TrimSpace(tagValue(m.Tags, "tick")); raw != "" {
		if parsed, err := strconv.ParseFloat(raw, 64); err == nil && parsed > 0 {
			w.tick = uint64(parsed)
		}
	}
}
...
func (w *World) SetModel(m *WorldModel) {
	...
	w.unitAbilityCandidates = w.unitAbilityCandidates[:0]
	...
	w.unitLookupNamesByID = nil
	w.unitLookupNamesByIndex = nil
	...
	if m != nil {
		...
		w.restoreRuntimeStateFromModelTagsLocked(m)
	}

	if m != nil && len(m.BlockNames) > 0 {
		w.blockNamesByID = make(map[int16]string, len(m.BlockNames))
		w.blockLookupNamesByID = make(map[int16]string, len(m.BlockNames))
		maxBlockID := int16(0)
		...
		for k, v := range m.BlockNames {
			...
			w.blockLookupNamesByID[k] = lookupName
			if k >= 0 {
				w.blockNamesByIndex[int(k)] = name
				w.blockLookupNamesByIndex[int(k)] = lookupName
	...
	if m != nil && len(m.Tiles) > 0 {
		if cap(w.itemSourceAccumByPos) < len(m.Tiles) {
			w.itemSourceAccumByPos = make([]float32, len(m.Tiles))
		} else {
			w.itemSourceAccumByPos = w.itemSourceAccumByPos[:len(m.Tiles)]
			clear(w.itemSourceAccumByPos)
		}
	}
	if m != nil && len(m.UnitNames) > 0 {
		w.unitNamesByID = make(map[int16]string, len(m.UnitNames))
		w.unitLookupNamesByID = make(map[int16]string, len(m.UnitNames))
		maxUnitID := int16(0)
		for k := range m.UnitNames {
			if k > maxUnitID {
				maxUnitID = k
			}
		}
		w.unitNamesByIndex = make([]string, int(maxUnitID)+1)
		w.unitLookupNamesByIndex = make([]string, int(maxUnitID)+1)
		for k, v := range m.UnitNames {
			name := strings.ToLower(strings.TrimSpace(v))
			lookupName := normalizeUnitName(name)
			w.unitNamesByID[k] = name
			w.unitLookupNamesByID[k] = lookupName
			if k >= 0 {
				w.unitNamesByIndex[int(k)] = name
				w.unitLookupNamesByIndex[int(k)] = lookupName
			}
		}
	...
	w.normalizeBuildingLiquidCurrentsLocked()
...
func (w *World) resolveUnitTypeIDLocked(want string) (int16, bool) {
	for id, name := range w.unitNamesByID {
		lookupName := w.unitLookupNameByID(id)
		if lookupName == "" {
			lookupName = normalizeUnitName(name)
		}
		if lookupName == want {
...
func (w *World) UnitNameByTypeID(typeID int16) string {
	w.mu.RLock()
	defer w.mu.RUnlock()
	if idx := int(typeID); idx >= 0 && idx < len(w.unitNamesByIndex) {
		if name := w.unitNamesByIndex[idx]; name != "" {
			return name
		}
	}
...
func (w *World) LoadVanillaProfiles(path string) error {
	...
	if len(payload.Turrets) > 0 {
		...
		for _, t := range payload.Turrets {
			...
			prepareBuildingWeaponProfile(name, &p)
			base[name] = p
		}
	...
	if w.model != nil && len(payload.Turrets) > 0 {
		w.rebuildBlockOccupancyLocked()
	}
	return nil
}
...
func cloneBuildingWeaponProfiles(src map[string]buildingWeaponProfile) map[string]buildingWeaponProfile {
	...
	for k, v := range src {
		...
		prepareBuildingWeaponProfile(k, &v)
		out[k] = v
	}
	return out
}

func prepareBuildingWeaponProfile(name string, p *buildingWeaponProfile) {
	if p == nil {
		return
	}
	p.BlockSyncKind = classifyTurretBlockSyncKindSlow(name, *p)
}
...
func applyVanillaTurretDefaults(p *buildingWeaponProfile) {
	if p == nil {
		return
	}
	className := strings.ToLower(strings.TrimSpace(p.ClassName))
	if !strings.Contains(className, "turret") || strings.Contains(className, "pointdefense") || strings.Contains(className, "tractorbeam") {
		return
	}
	p.Rotate = true
	if p.RotateSpeed <= 0 {
	...
	if p.TargetInterval <= 0 {
		p.TargetInterval = 20.0 / 60.0
	}
	if p.TargetSwitchInterval <= 0 {
		p.TargetSwitchInterval = p.TargetInterval
	}
}
...
func (w *World) applyEntityBoundsRepulsionLocked(e *RawEntity, dt float32) bool {
	if w == nil || w.model == nil || e == nil || dt <= 0 {
		return false
	}
	worldW := float32(w.model.Width * 8)
	worldH := float32(w.model.Height * 8)
	if worldW <= 0 || worldH <= 0 {
		return false
	}
	const warpDst = float32(8)
	deltaTicks := dt * 60
	...
	if limit := worldW - 8; e.X > limit {
		vx -= (e.X - limit) / warpDst
	}
	if limit := worldH - 8; e.Y > limit {
		vy -= (e.Y - limit) / warpDst
	}
	changed := false
	if vx != 0 || vy != 0 {
		e.VelX += vx * deltaTicks
		e.VelY += vy * deltaTicks
		changed = true
	}
	clampedX := clampf(e.X, -8, worldW)
	clampedY := clampf(e.Y, -8, worldH)
	if clampedX != e.X || clampedY != e.Y {
		e.X, e.Y = clampedX, clampedY
		changed = true
	}
	return changed
}
...
func (w *World) stepEntities(delta time.Duration) (movementDur, combatDur, buildingCombatDur, bulletDur time.Duration) {
	...
	for i := 0; i < len(w.model.Entities); {
		...
		if w.applyEntityBoundsRepulsionLocked(e, dt) {
			changed = true
		}
		outCheckX, outCheckY := e.X, e.Y
		if w.applyVanillaFlyingWobbleLocked(e, dt*60) {
			changed = true
		}
		if e.RotVel != 0 {
		...
		out := outCheckX < 0 || outCheckY < 0 || outCheckX > maxX || outCheckY > maxY
		if out && e.PlayerID != 0 {
	...
	// Movement/死亡可能增删实体：重建 ID 索引与空间索引。
	clear(idToIndex)
	for i := range w.model.Entities {
		idToIndex[w.model.Entities[i].ID] = i
	}
	...
	w.stepBullets(dt, idToIndex, spatial, teamSpatial, w.unitAbilityCandidates)
	bulletDur = time.Since(bulletStartedAt)
	return movementDur, combatDur, buildingCombatDur, bulletDur
}

func (w *World) applyVanillaFlyingWobbleLocked(e *RawEntity, deltaFrames float32) bool {
	if w == nil || e == nil || e.Health <= 0 || e.PlayerID != 0 || deltaFrames <= 0 || !isEntityFlying(*e) {
		return false
	}
	switch w.unitAIKindForEntityLocked(*e) {
	case unitAIBuilder:
		return false
	}
	...
	phase := w.timeSec*60 + float32(e.ID%10)*12
	dx := arcSinScaled(phase, 25, 0.05) * deltaFrames * elevation
	dy := arcCosScaled(phase, 25, 0.05) * deltaFrames * elevation
	if dx == 0 && dy == 0 {
		return false
	}
	e.X += dx
...
func (w *World) stepBuildingCombat(dt float32, idToIndex map[int32]int, spatial *entitySpatialIndex, teamSpatial map[TeamID]*entitySpatialIndex) {
	...
	for _, pos := range w.turretTilePositions {
		...
		if !exists {
			state = initialBuildCombatStateFromTile(t, prof)
		} else if !state.HasRotation {
			if _, rotation, ok := decodeTurretMapSyncTail(t); ok {
				state.TurretRotation = normalizeAngleDeg(rotation)
			} else {
		...
		if state.Cooldown > 0 {
			...
			if state.Cooldown > 0 {
				name := w.blockNameByID(int16(t.Build.Block))
				if _, _, advance, ok := w.consumeTurretCoolantLocked(pos, t, name, scaledDT*60); ok {
					state.Cooldown = maxf(0, state.Cooldown-advance)
				}
			}
		}
...
func (w *World) rebuildBuildingProfileBlockCacheLocked() {
	if w == nil {
		return
	}
	...
	if cap(w.buildingProfilesByBlock) < len(w.blockNamesByIndex) {
		w.buildingProfilesByBlock = make([]buildingWeaponProfile, len(w.blockNamesByIndex))
		w.buildingProfileBlockState = make([]uint8, len(w.blockNamesByIndex))
	} else {
		w.buildingProfilesByBlock = w.buildingProfilesByBlock[:len(w.blockNamesByIndex)]
		w.buildingProfileBlockState = w.buildingProfileBlockState[:len(w.blockNamesByIndex)]
		clear(w.buildingProfileBlockState)
	}
	src := w.buildingProfilesByName
	if len(src) == 0 {
		src = buildingWeaponProfilesByName
	}
	for id, name := range w.blockNamesByIndex {
		if id == 0 || name == "" {
			continue
		}
		if prof, ok := src[name]; ok {
			w.buildingProfilesByBlock[id] = prof
			w.buildingProfileBlockState[id] = 1
			continue
		}
		normalized := strings.ToLower(strings.TrimSpace(name))
		if normalized == "" || normalized == name {
...
func (w *World) forEachEnemyBuildingInRange(team TeamID, x, y, radius float32, visit func(pos int32)) {
	...
	if len(w.teamBuildingSpatialEntries) != 0 {
		for _, entry := range w.teamBuildingSpatialEntries {
			if entry.Team == 0 || entry.Team == team || entry.Idx == nil {
				continue
			}
			entry.Idx.forEachInRange(x, y, radius, visit)
		}
		return
	}
...
func (idx *buildingSpatialIndex) insert(tileX, tileY int, pos int32) {
	...
	if cell := idx.gridCell(cx, cy); cell != nil {
		*cell = append(*cell, pos)
		return
	}
	key := packSpatialCell(cx, cy)
...
func (w *World) teamBuildingSpatialIndexLocked(team TeamID) *buildingSpatialIndex {
	if w == nil || team == 0 {
		return nil
	}
	...
	idx = newBuildingSpatialIndexLocked(w)
	w.teamBuildingSpatial[team] = idx
	w.teamBuildingSpatialEntries = append(w.teamBuildingSpatialEntries, teamBuildingSpatialEntry{Team: team, Idx: idx})
	return idx
}

func newBuildingSpatialIndexLocked(w *World) *buildingSpatialIndex {
	idx := &buildingSpatialIndex{cellSize: 64, cells: map[int64][]int32{}}
	if w == nil || w.model == nil || w.model.Width <= 0 || w.model.Height <= 0 {
		return idx
	}
	idx.gridW = (w.model.Width*8 + idx.cellSize - 1) / idx.cellSize
	idx.gridH = (w.model.Height*8 + idx.cellSize - 1) / idx.cellSize
	if idx.gridW <= 0 || idx.gridH <= 0 {
		return idx
	}
	idx.grid = make([][]int32, idx.gridW*idx.gridH)
	return idx
}
...
func (idx *buildingSpatialIndex) gridCell(cx, cy int) *[]int32 {
	if idx == nil || idx.gridW <= 0 || idx.gridH <= 0 || cx < 0 || cy < 0 || cx >= idx.gridW || cy >= idx.gridH || len(idx.grid) == 0 {
		return nil
	}
	return &idx.grid[cy*idx.gridW+cx]
}
...
func (idx *buildingSpatialIndex) forEachInRange(x, y, radius float32, visit func(pos int32)) {
	...
	if len(idx.grid) > 0 && idx.gridW > 0 && idx.gridH > 0 {
		minXF := (x - radius) / cell
		maxXF := (x + radius) / cell
		minYF := (y - radius) / cell
		maxYF := (y + radius) / cell
		if maxXF < 0 || maxYF < 0 || minXF >= float32(idx.gridW) || minYF >= float32(idx.gridH) {
			return
		}
		minCX := int(minXF)
		maxCX := int(maxXF)
		...
		for cy := minCY; cy <= maxCY; cy++ {
			...
			for cx := minCX; cx <= maxCX; cx++ {
				for _, pos := range idx.grid[row+cx] {
					visit(pos)
				}
			}
		}
...
func normalizeAngle360Deg(v float32) float32 {
	for v < 0 {
		v += 360
	}
	for v >= 360 {
...
// addEnemy 添加敌方单位
func (w *World) addEnemy(unitType int16, team TeamID, x, y, rotation, shield float32) {
	w.addEnemyInternal(unitType, team, x, y, rotation, shield, false)
}

func (w *World) addWaveEnemy(unitType int16, team TeamID, x, y, rotation, shield float32) {
	w.addEnemyInternal(unitType, team, x, y, rotation, shield, true)
}

func (w *World) addEnemyInternal(unitType int16, team TeamID, x, y, rotation, shield float32, vanillaWave bool) {
	if w.model == nil {
		return
	}
	...
	if vanillaWave {
		unit.Shield = shield
		unit.ControllerType = w.vanillaAIControllerTypeForUnitLocked(unitType)
		w.applyStatusToEntity(&unit, -1, "unmoving", 30.0/60.0)
		w.applyStatusToEntity(&unit, -1, "invincible", 60.0/60.0)
	} else if shield > 0 {
		unit.Shield = shield
	}
