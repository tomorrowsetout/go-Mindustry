...
func TestDumpProximityUsesTraceProxyOrder(t *testing.T) {
	w := New(Config{TPS: 60})
	...
	src := placeTestBuilding(t, w, 4, 4, 1, 1, 0)
	east := placeTestBuilding(t, w, 5, 4, 1, 1, 0)
	north := placeTestBuilding(t, w, 4, 3, 1, 1, 0)
	west := placeTestBuilding(t, w, 3, 4, 1, 1, 0)
	south := placeTestBuilding(t, w, 4, 5, 1, 1, 0)

	got := w.dumpProximityLocked(int32(src.Y*model.Width + src.X))
	vanillaOrder := []int32{
		int32(east.Y*model.Width + east.X),
		int32(south.Y*model.Width + south.X),
		int32(west.Y*model.Width + west.X),
		int32(north.Y*model.Width + north.X),
	}
	want := sortedDumpNeighborsForSeed(vanillaOrder, defaultDumpProximityProxySeed)
	if len(got) != len(want) {
		t.Fatalf("expected %d dump neighbors, got %d: %v", len(want), len(got), got)
	}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("expected trace proxy dump neighbor order %v, got %v", want, got)
		}
	}
}
...
func TestStatusDamageUsesSequentialVanillaMultipliers(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(8, 8)
	w.SetModel(model)
	w.statusProfilesByID[1] = statusEffectProfile{
	...
	w.statusProfilesByID[13] = statusEffectProfile{
		ID:                   13,
		Name:                 "overdrive",
		DamageMultiplier:     1,
	...
	w.statusProfilesByID[15] = statusEffectProfile{
		ID:                   15,
		Name:                 "shielded",
		DamageMultiplier:     1,
		HealthMultiplier:     3,
	...
	w.statusProfilesByID[16] = statusEffectProfile{
		ID:                   16,
		Name:                 "boss",
		DamageMultiplier:     1,
	...
	for _, prof := range w.statusProfilesByID {
		w.statusProfilesByName[prof.Name] = prof
	}
	...
	entity := RawEntity{
		...
		Statuses: []entityStatusState{
			{ID: 13, Name: "overdrive", Time: 1},
			{ID: 1, Name: "burning", Time: 1},
			{ID: 15, Name: "shielded", Time: 1},
			{ID: 16, Name: "boss", Time: 1},
		},
	}
	w.updateEntityStatuses(&entity, 1.0/60.0)

	expectedShield := float32(10 - 1/0.95)
	if math.Abs(float64(entity.Shield-expectedShield)) > 0.0001 {
		t.Fatalf("expected burning DOT to use only preceding health multipliers, got shield=%f want=%f", entity.Shield, expectedShield)
	}
	expectedHealthMul := float32(0.95 * 3 * 1.5)
	if math.Abs(float64(entity.StatusHealthMul-expectedHealthMul)) > 0.0001 {
		t.Fatalf("expected final health multiplier to include all statuses, got=%f want=%f", entity.StatusHealthMul, expectedHealthMul)
	}
}
...
func TestMechanicalPumpDumpsLiquidByVanillaPressureFraction(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(12, 12)
	model.BlockNames = map[int16]string{
		1:   "water",
	...
	paintAreaFloor(t, w, 4, 4, 1, 1)
	pump := placeTestBuilding(t, w, 4, 4, 440, 1, 0)
	conduit := placeTestBuilding(t, w, 5, 4, 257, 1, 0)
	w.rebuildActiveTilesLocked()

	stepWorldFrames(w, 1)

	if got := pump.Build.LiquidAmount(waterLiquidID); math.Abs(float64(got-(7.0/60.0/2.0))) > 0.0001 {
		t.Fatalf("expected pump to retain half of first-frame water, got %f", got)
	}
	if got := conduit.Build.LiquidAmount(waterLiquidID); math.Abs(float64(got-(7.0/60.0/2.0))) > 0.0001 {
		t.Fatalf("expected conduit to receive pressure-fraction water, got %f", got)
	}
}

func TestMechanicalPumpCanCoolAdjacentTurret(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(12, 12)
	model.BlockNames = map[int16]string{
		1:   "water",
	...
	paintAreaFloor(t, w, 4, 4, 1, 1)
	pump := placeTestBuilding(t, w, 4, 4, 440, 1, 0)
	turret := placeTestBuilding(t, w, 5, 4, 911, 1, 0)
	w.rebuildActiveTilesLocked()
	...
	if got := turret.Build.LiquidAmount(waterLiquidID); math.Abs(float64(got-(7.0/60.0/2.0))) > 0.0001 {
		t.Fatalf("expected adjacent turret to receive coolant water, got %f", got)
	}
}

func TestAdjacentMechanicalPumpsDoNotAcceptDumpedLiquid(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(12, 12)
	model.BlockNames = map[int16]string{
		1:   "water",
	...
	w.SetModel(model)
	paintAreaFloor(t, w, 4, 4, 2, 1)
	left := placeTestBuilding(t, w, 4, 4, 440, 1, 0)
	right := placeTestBuilding(t, w, 5, 4, 440, 1, 0)
	w.rebuildActiveTilesLocked()

	stepWorldFrames(w, 1)

	want := float32(7.0 / 60.0)
	if got := left.Build.LiquidAmount(waterLiquidID); math.Abs(float64(got-want)) > 0.0001 {
		t.Fatalf("expected left pump to keep only its own production, got %f", got)
	}
	if got := right.Build.LiquidAmount(waterLiquidID); math.Abs(float64(got-want)) > 0.0001 {
		t.Fatalf("expected right pump to keep only its own production, got %f", got)
	}
}
...
func TestLiquidSourceFillsAdjacentTurretByVanillaPressure(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(8, 8)
	model.BlockNames = map[int16]string{
		355: "arc",
	...
	source := placeTestBuilding(t, w, 3, 3, 610, 1, 0)
	turret := placeTestBuilding(t, w, 4, 3, 355, 1, 0)
	w.applyBuildingConfigLocked(int32(source.Y*model.Width+source.X), protocolContentLiquid(waterLiquidID), false)

	w.Step(time.Second / 60)

	if got := turret.Build.LiquidAmount(waterLiquidID); math.Abs(float64(got-20)) > 0.0001 {
		t.Fatalf("expected liquid-source to fill adjacent turret capacity, got %f", got)
	}
	if got := source.Build.LiquidAmount(waterLiquidID); math.Abs(float64(got-9980)) > 0.0001 {
		t.Fatalf("expected liquid-source to retain vanilla source capacity minus transfer, got %f", got)
	}
}

func TestLiquidSourceUsesVanillaLiquidTurretCapacity(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(8, 8)
	model.BlockNames = map[int16]string{
		353: "wave",
	...
	source := placeTestBuilding(t, w, 3, 3, 610, 1, 0)
	turret := placeTestBuilding(t, w, 4, 3, 353, 1, 0)
	w.applyBuildingConfigLocked(int32(source.Y*model.Width+source.X), protocolContentLiquid(waterLiquidID), false)

	w.Step(time.Second / 60)

	if got := turret.Build.LiquidAmount(waterLiquidID); math.Abs(float64(got-10)) > 0.0001 {
		t.Fatalf("expected wave liquid capacity to match vanilla 10, got %f", got)
	}
	if got := source.Build.LiquidAmount(waterLiquidID); math.Abs(float64(got-9990)) > 0.0001 {
		t.Fatalf("expected liquid-source to retain vanilla source capacity minus wave transfer, got %f", got)
	}
}

func TestLiquidSourceCanCoolAdjacentForceProjector(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(10, 10)
	model.BlockNames = map[int16]string{
		249: "force-projector",
	...
	source := placeTestBuilding(t, w, 4, 4, 610, 1, 0)
	projector := placeTestBuilding(t, w, 6, 4, 249, 1, 0)
	w.applyBuildingConfigLocked(int32(source.Y*model.Width+source.X), protocolContentLiquid(cryofluidLiquidID), false)

	w.Step(time.Second / 60)

	if got := projector.Build.LiquidAmount(cryofluidLiquidID); math.Abs(float64(got-60)) > 0.0001 {
		t.Fatalf("expected force-projector cryofluid capacity to match vanilla 60, got %f", got)
	}
	if got := source.Build.LiquidAmount(cryofluidLiquidID); math.Abs(float64(got-9940)) > 0.0001 {
		t.Fatalf("expected liquid-source to retain vanilla source capacity minus force-projector transfer, got %f", got)
	}
}

func TestBuildingTurretConsumesCryofluidCoolantWhileReloading(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(8, 8)
	model.BlockNames = map[int16]string{
		365: "spectre",
	...
	w.SetModel(model)
	turret := placeTestBuilding(t, w, 4, 4, 365, 1, 0)
	turret.Build.AddLiquid(cryofluidLiquidID, 120)

	pos := int32(turret.Y*model.Width + turret.X)
	w.buildStates[pos] = buildCombatState{Cooldown: 1}
	w.stepBuildingCombat(0.016, nil, nil, nil)

	if got := turret.Build.LiquidAmount(cryofluidLiquidID); math.Abs(float64(got-119.04)) > 0.0001 {
		t.Fatalf("expected spectre to consume vanilla cryofluid amount for 16ms tick, got %f", got)
	}
	if got := w.buildStates[pos].Cooldown; got >= 1 || got <= 0 {
		t.Fatalf("expected cryofluid to advance spectre reload without finishing it, cooldown=%f", got)
	}
}

func TestBuildingTurretDoesNotConsumeCoolantAfterNaturalReloadCompletes(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(8, 8)
	model.BlockNames = map[int16]string{
		365: "spectre",
	...
	pos := int32(turret.Y*model.Width + turret.X)
	w.buildStates[pos] = buildCombatState{Cooldown: 0.01}
	w.stepBuildingCombat(1.0/60.0, nil, nil, nil)

	if got := turret.Build.LiquidAmount(cryofluidLiquidID); math.Abs(float64(got-120)) > 0.0001 {
		t.Fatalf("expected full reload to skip coolant consumption, got %f", got)
	}
}

func TestBuildingTurretSkipsSameTickCoolantDeliveredByLiquidSource(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(8, 8)
	model.BlockNames = map[int16]string{
		355: "arc",
	...
	turret := placeTestBuilding(t, w, 4, 4, 355, 1, 0)
	source := placeTestBuilding(t, w, 5, 4, 610, 1, 0)
	w.applyBuildingConfigLocked(int32(source.Y*model.Width+source.X), protocolContentLiquid(waterLiquidID), false)
	pos := int32(turret.Y*model.Width + turret.X)
	w.buildStates[pos] = buildCombatState{Cooldown: 1}
	...
	if got := turret.Build.LiquidAmount(waterLiquidID); math.Abs(float64(got-20)) > 0.0001 {
		t.Fatalf("expected same-tick delivered coolant to remain available for the next tick, got %f", got)
	}
}

func TestLiquidSourceDoesNotAcceptDumpedLiquid(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(8, 8)
	model.BlockNames = map[int16]string{
		431: "liquid-router",
	...
	src := placeTestBuilding(t, w, 3, 3, 431, 1, 0)
	source := placeTestBuilding(t, w, 4, 3, 610, 1, 0)
	src.Build.AddLiquid(waterLiquidID, 10)

	moved := w.tryMoveLiquidLocked(int32(src.Y*model.Width+src.X), int32(source.Y*model.Width+source.X), waterLiquidID, 10, 0)
	if moved != 0 {
		t.Fatalf("expected liquid-source to reject incoming liquid, moved=%f", moved)
	}
	if got := source.Build.LiquidAmount(waterLiquidID); got != 0 {
		t.Fatalf("expected liquid-source to stay empty without source config tick, got %f", got)
	}
}
...
func TestOverdriveProjectorAcceptsPhaseFabricBoostItem(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(8, 8)
	model.BlockNames = map[int16]string{
	...
	src := placeTestBuilding(t, w, 3, 3, 901, 1, 0)
	projector := placeTestBuilding(t, w, 4, 3, 900, 1, 0)
	src.Build.AddItem(phaseFabricItemID, 1)

	if !w.tryInsertItemLocked(int32(src.Y*model.Width+src.X), int32(projector.Y*model.Width+projector.X), phaseFabricItemID, 0) {
		t.Fatal("expected overdrive-projector to accept phase fabric boost item")
	}
	if got := projector.Build.ItemAmount(phaseFabricItemID); got != 1 {
		t.Fatalf("expected overdrive-projector to store phase fabric, got %d", got)
	}
}

func TestItemSourceFeedsAdjacentBoostConsumerBeforeOtherStorage(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(10, 10)
	model.BlockNames = map[int16]string{
		412: "item-source",
	...
	source := placeTestBuilding(t, w, 4, 4, 412, 1, 0)
	projector := placeTestBuilding(t, w, 5, 4, 900, 1, 0)
	router := placeTestBuilding(t, w, 4, 5, 418, 1, 0)
	w.ConfigureItemSource(int32(source.Y*model.Width+source.X), phaseFabricItemID)

	w.Step(time.Second / 60)

	if got := projector.Build.ItemAmount(phaseFabricItemID); got != 1 {
		neighbors := w.dumpProximityLocked(int32(source.Y*model.Width + source.X))
		t.Fatalf("expected item-source to feed adjacent overdrive-projector first, got projector=%d router=%d neighbors=%v", got, router.Build.ItemAmount(phaseFabricItemID), neighbors)
	}
	if got := router.Build.ItemAmount(phaseFabricItemID); got != 0 {
		t.Fatalf("expected router to stay empty while boost consumer accepts item, got %d", got)
	}
}
...
func TestLinkedStorageMergesIntoCoreInventory(t *testing.T) {
	...
	if store.Build.ItemAmount(0) != 4 {
		t.Fatalf("expected linked vault to expose shared core inventory, got %d", store.Build.ItemAmount(0))
	}
...
func TestMultipleCoresExposeSharedInventoryLikeVanilla(t *testing.T) {
	w := New(Config{TPS: 60})
	...
	first := placeTestBuilding(t, w, 5, 5, 339, 1, 0)
	second := placeTestBuilding(t, w, 12, 5, 339, 1, 0)
	first.Build.Items = []ItemStack{{Item: copperItemID, Amount: 2}}
	second.Build.Items = []ItemStack{{Item: copperItemID, Amount: 3}}
	w.rebuildBlockOccupancyLocked()

	if got := first.Build.ItemAmount(copperItemID); got != 5 {
		t.Fatalf("expected first core to expose merged shared copper, got %d", got)
	}
	if got := second.Build.ItemAmount(copperItemID); got != 5 {
		t.Fatalf("expected second core to expose merged shared copper, got %d", got)
	}

	secondPos := int32(second.Y*model.Width + second.X)
	if !w.addItemAtLocked(secondPos, siliconItemID, 4) {
		t.Fatal("expected shared core add to succeed")
	}
	if got := first.Build.ItemAmount(siliconItemID); got != 4 {
		t.Fatalf("expected first core to see silicon added through second core, got %d", got)
	}
	if got := second.Build.ItemAmount(siliconItemID); got != 4 {
		t.Fatalf("expected second core to see silicon added through shared inventory, got %d", got)
	}
	if !w.removeItemAtLocked(secondPos, copperItemID, 1) {
		t.Fatal("expected shared core removal to succeed")
	}
	if got := first.Build.ItemAmount(copperItemID); got != 4 {
		t.Fatalf("expected first core to see copper removed through second core, got %d", got)
	}
	if got := second.Build.ItemAmount(copperItemID); got != 4 {
		t.Fatalf("expected second core to see copper removed through shared inventory, got %d", got)
	}
}
...
func TestBlockEdgeOffsetsMatchVanillaEdgesOrder(t *testing.T) {
	tests := []struct {
		name string
		size int
		want [][2]int
		...
		},
		{
			name: "size 2",
			size: 2,
			want: [][2]int{{2, 0}, {2, 1}, {1, 2}, {0, 2}, {-1, 1}, {-1, 0}, {0, -1}, {1, -1}},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := blockEdgeOffsets(tt.size)
			if len(got) != len(tt.want) {
				t.Fatalf("expected %d offsets, got %d: %v", len(tt.want), len(got), got)
			}
			for i := range tt.want {
				if got[i] != tt.want[i] {
					t.Fatalf("offset[%d] expected %v, got %v; full=%v", i, tt.want[i], got[i], got)
				}
			}
		})
	}
...
func TestInitialBuildCombatStateUsesTurretMapSyncTail(t *testing.T) {
	raw := protocol.NewWriter()
	_ = raw.WriteFloat32(0.1)
	_ = raw.WriteFloat32(90)
	tile := &Tile{
		Rotation: 2,
		Build: &Building{
			MapSyncRevision: 1,
			MapSyncTail:     append([]byte(nil), raw.Bytes()...),
		},
	}
	state := initialBuildCombatStateFromTile(tile, buildingWeaponProfile{
		Interval:      0.6,
		AmmoCapacity:  80,
		PowerCapacity: 140,
	})
	if math.Abs(float64(state.TurretRotation-90)) > 0.001 {
		t.Fatalf("expected map sync turret rotation 90, got %f", state.TurretRotation)
	}
	if math.Abs(float64(state.Cooldown-0.5)) > 0.001 {
		t.Fatalf("expected cooldown from reload counter, got %f", state.Cooldown)
	}
	if state.Ammo != 80 || state.Power != 140 || !state.HasRotation {
		t.Fatalf("expected capacity defaults to survive map sync init, got %+v", state)
	}
}

func TestBuildingTurretMapSyncRotationSyncsWithoutTarget(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(16, 16)
	model.BlockNames = map[int16]string{
		911: "arc",
	...
	tile := placeTestBuilding(t, w, 8, 8, 911, 1, 2)
	raw := protocol.NewWriter()
	_ = raw.WriteFloat32(0)
	_ = raw.WriteFloat32(90)
	tile.Build.MapSyncRevision = 1
	tile.Build.MapSyncTail = append([]byte(nil), raw.Bytes()...)
	w.rebuildActiveTilesLocked()

	stepWorldFrames(w, 1)

	if got := tile.Rotation; got != 1 {
		t.Fatalf("expected map sync turret head rotation to sync cardinal rotation 1, got %d", got)
	}
}

func TestAntiAirTurretDoesNotTargetBuildingsFromHitBuildingsOnly(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(16, 16)
	model.BlockNames = map[int16]string{
		910: "scatter",
	...
	w.SetModel(model)
	w.buildingProfilesByName["scatter"] = buildingWeaponProfile{
		ClassName:    "ItemTurret",
		Range:        220,
		Damage:       45,
		Interval:     0.3,
	...
	tile := placeTestBuilding(t, w, 8, 8, 910, 1, 0)
	raw := protocol.NewWriter()
	_ = raw.WriteFloat32(0)
	_ = raw.WriteFloat32(90)
	tile.Build.MapSyncRevision = 1
	tile.Build.MapSyncTail = append([]byte(nil), raw.Bytes()...)
	placeTestBuilding(t, w, 4, 8, 600, 2, 0)
	w.rebuildActiveTilesLocked()

	stepWorldFrames(w, 1)

	if got := tile.Rotation; got != 1 {
		t.Fatalf("expected anti-air turret to keep map-sync rotation instead of targeting building, got %d", got)
	}
}
...
func TestCloneModelForWorldStreamInfersUnknownMultiblockEdges(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(16, 16)
	model.BlockNames = map[int16]string{
		900: "custom-4x4-block",
	}
	w.SetModel(model)
	center := placeTestBuilding(t, w, 5, 5, 900, 2, 1)
	low, high := blockFootprintRange(4)
	for y := center.Y + low; y <= center.Y+high; y++ {
		for x := center.X + low; x <= center.X+high; x++ {
			if x == center.X && y == center.Y {
				continue
			}
			tile, err := w.Model().TileAt(x, y)
			if err != nil || tile == nil {
				t.Fatalf("edge tile lookup failed at (%d,%d): %v", x, y, err)
			}
			tile.Block = 900
		}
	}
	...
	edgeTile, err := clone.TileAt(4, 4)
	if err != nil || edgeTile == nil {
		t.Fatalf("edge tile lookup failed: %v", err)
	}
	if edgeTile.Block != 900 || edgeTile.Team != 2 || edgeTile.Rotation != 1 {
		t.Fatalf("expected inferred edge to mirror center block/team/rotation, got block=%d team=%d rot=%d", edgeTile.Block, edgeTile.Team, edgeTile.Rotation)
	}
	centerClone, err := clone.TileAt(5, 5)
	if err != nil || centerClone == nil || centerClone.Build == nil {
		t.Fatalf("center tile lookup failed: %v", err)
	}
	if edgeTile.Build != centerClone.Build {
		t.Fatal("expected inferred edge to share center build")
	}
}
...
func TestLoadVanillaProfilesRefreshesTurretIndexForProfileOnlyTurret(t *testing.T) {
	profilesPath := filepath.Join("..", "..", "data", "vanilla", "profiles.json")
	if _, err := os.Stat(profilesPath); err != nil {
		t.Skipf("vanilla profiles fixture not available: %v", err)
	}
	w := New(Config{TPS: 60})
	delete(w.buildingProfilesByName, "arc")
	model := NewWorldModel(24, 24)
	model.BlockNames = map[int16]string{
		355: "arc",
		600: "copper-wall",
	...
	w.SetModel(model)
	arc := placeTestBuilding(t, w, 10, 10, 355, 2, 2)
	target := placeTestBuilding(t, w, 10, 14, 600, 1, 0)
	target.Build.Health = 100
	target.Build.MaxHealth = 100
	w.rebuildActiveTilesLocked()
	arcPos := int32(arc.Y*model.Width + arc.X)
	if containsPos(w.turretTilePositions, arcPos) {
		t.Fatalf("expected arc to be unindexed before profile load")
	}

	if err := w.LoadVanillaProfiles(profilesPath); err != nil {
		t.Fatalf("load vanilla profiles: %v", err)
	}
	if !containsPos(w.turretTilePositions, arcPos) {
		t.Fatalf("expected arc to be indexed as turret after profile load")
	}

	stepWorldFrames(w, 20)

	if got := w.model.Tiles[arcPos].Rotation; got != 1 {
		t.Fatalf("expected arc to progressively rotate toward vanilla building target, got %d", got)
	}
}
...
func TestOverdriveProjectorUsesPhaseFabricBoostItem(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(20, 20)
	model.BlockNames = map[int16]string{
		900: "overdrive-projector",
	...
	projector := placeTestBuilding(t, w, 8, 8, 900, 1, 0)
	press := placeTestBuilding(t, w, 10, 8, 901, 1, 0)
	projector.Build.AddItem(phaseFabricItemID, 2)
	w.rebuildActiveTilesLocked()

	projectorPos := int32(projector.Y*w.Model().Width + projector.X)
	pressPos := int32(press.Y*w.Model().Width + press.X)
	state := w.overdriveProjectorStateLocked(projectorPos)
	state.Charge = overdriveProjectorProfiles["overdrive-projector"].Reload
	state.UseProgress = overdriveProjectorProfiles["overdrive-projector"].UseTime - 0.5

	w.Step(time.Second / 60)

	if got := state.PhaseHeat; math.Abs(float64(got-0.1)) > 0.0001 {
		t.Fatalf("expected phase heat to lerp toward optional efficiency, got %f", got)
	}
	if got := w.buildingTimeScaleLocked(pressPos); got <= overdriveProjectorProfiles["overdrive-projector"].SpeedBoost {
		t.Fatalf("expected phase fabric to raise boost above base speed boost, timeScale=%f", got)
	}
	if got := projector.Build.ItemAmount(phaseFabricItemID); got != 1 {
		t.Fatalf("expected overdrive projector to consume one phase fabric at use time, got %d", got)
	}
	if got := state.UseProgress; math.Abs(float64(got-0.5)) > 0.0001 {
		t.Fatalf("expected overdrive use progress to keep vanilla modulo remainder, got %f", got)
	}
}

func TestOverdriveProjectorPhaseHeatDecaysWithoutBoostItem(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(8, 8)
	model.BlockNames = map[int16]string{
		900: "overdrive-projector",
	...
	projector := placeTestBuilding(t, w, 4, 4, 900, 1, 0)
	w.rebuildActiveTilesLocked()
	projectorPos := int32(projector.Y*w.Model().Width + projector.X)
	state := w.overdriveProjectorStateLocked(projectorPos)
	state.PhaseHeat = 1
	...
	if got := state.PhaseHeat; math.Abs(float64(got-0.9)) > 0.0001 {
		t.Fatalf("expected phase heat to decay toward missing optional item, got %f", got)
	}
}

func TestOverdriveDomeRequiresConfiguredItemsToBoost(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(20, 20)
	model.BlockNames = map[int16]string{
		900: "overdrive-dome",
	...
	dome := placeTestBuilding(t, w, 8, 8, 900, 1, 0)
	press := placeTestBuilding(t, w, 10, 8, 901, 1, 0)
	w.rebuildActiveTilesLocked()

	domePos := int32(dome.Y*w.Model().Width + dome.X)
	pressPos := int32(press.Y*w.Model().Width + press.X)
	state := w.overdriveProjectorStateLocked(domePos)
	state.Charge = overdriveProjectorProfiles["overdrive-dome"].Reload

	w.Step(time.Second / 60)

	if got := w.buildingTimeScaleLocked(pressPos); got != 1 {
		t.Fatalf("expected item-starved overdrive dome to stay inactive, timeScale=%f", got)
	}

	dome.Build.AddItem(phaseFabricItemID, 1)
	dome.Build.AddItem(siliconItemID, 1)
	state.Charge = overdriveProjectorProfiles["overdrive-dome"].Reload
	state.UseProgress = overdriveProjectorProfiles["overdrive-dome"].UseTime

	w.Step(time.Second / 60)

	if got := w.buildingTimeScaleLocked(pressPos); got <= 1 {
		t.Fatalf("expected stocked overdrive dome to boost nearby buildings, timeScale=%f", got)
	}
	if got := dome.Build.ItemAmount(phaseFabricItemID); got != 0 {
		t.Fatalf("expected overdrive dome to consume phase fabric, got %d", got)
	}
	if got := dome.Build.ItemAmount(siliconItemID); got != 0 {
		t.Fatalf("expected overdrive dome to consume silicon, got %d", got)
	}
}
...
func TestMendProjectorConsumesPhaseFabricBoostItem(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(20, 20)
	model.BlockNames = map[int16]string{
		910: "mend-projector",
	...
	projector := placeTestBuilding(t, w, 8, 8, 910, 1, 0)
	target := placeTestBuilding(t, w, 10, 8, 911, 1, 0)
	target.Build.Health = 500
	target.Build.MaxHealth = 1000
	projector.Build.AddItem(phaseFabricItemID, 2)
	w.rebuildActiveTilesLocked()

	projectorPos := int32(projector.Y*model.Width + projector.X)
	state := w.mendProjectorStateLocked(projectorPos)
	state.UseProgress = mendProjectorProfiles["mend-projector"].UseTime - 0.5

	w.Step(time.Second / 60)

	if got := state.PhaseHeat; math.Abs(float64(got-0.1)) > 0.0001 {
		t.Fatalf("expected mend-projector phase heat to track optional efficiency, got %f", got)
	}
	if got := projector.Build.ItemAmount(phaseFabricItemID); got != 1 {
		t.Fatalf("expected mend-projector to consume one phase fabric at use time, got %d", got)
	}
	if got := state.UseProgress; math.Abs(float64(got-0.5)) > 0.0001 {
		t.Fatalf("expected mend-projector use progress to keep modulo remainder, got %f", got)
	}
}

func TestForceProjectorConsumesPhaseFabricBoostItem(t *testing.T) {
	w := New(Config{TPS: 60})
	model := NewWorldModel(20, 20)
	model.BlockNames = map[int16]string{
		920: "force-projector",
	...
	projector := placeTestBuilding(t, w, 8, 8, 920, 1, 0)
	projector.Build.AddItem(phaseFabricItemID, 2)
	w.rebuildActiveTilesLocked()

	projectorPos := int32(projector.Y*model.Width + projector.X)
	state := w.forceProjectorStateLocked(projectorPos)
	state.PhaseUseProgress = forceProjectorProfiles["force-projector"].PhaseUseTime - 0.5

	w.Step(time.Second / 60)

	if got := state.PhaseHeat; math.Abs(float64(got-0.1)) > 0.0001 {
		t.Fatalf("expected force-projector phase heat to track phase fabric, got %f", got)
	}
	if got := projector.Build.ItemAmount(phaseFabricItemID); got != 1 {
		t.Fatalf("expected force-projector to consume one phase fabric at use time, got %d", got)
	}
	if got := state.PhaseUseProgress; math.Abs(float64(got-0.5)) > 0.0001 {
		t.Fatalf("expected force-projector phase use progress to keep modulo remainder, got %f", got)
	}
}
