...
// stepEntityAbilities advances unit abilities and reports whether any entity
// position changed (shield-arc push). Callers can skip spatial index rebuild
// when this returns false.
func (w *World) stepEntityAbilities(dt float32) (movedEntities bool) {
	...
	for i := range w.model.Entities {
		...
		for ai, ability := range prof.Abilities {
			...
			if w.stepEntityAbility(e, &e.Abilities[ai], ability, dt) {
				changed = true
				if ability.Kind == unitAbilityShieldArc {
					// Shield-arc is the only ability that can push other units.
					movedEntities = true
				}
			}
		}
...
func (w *World) collectUnitAbilityCandidatesLocked(dst []unitAbilityCandidate) []unitAbilityCandidate {
	dst = dst[:0]
	...
	for i := range w.model.Entities {
		...
		for ai, ability := range prof.Abilities {
			...
			switch ability.Kind {
			case unitAbilityForceField:
				if e.Shield > 0 && ability.Radius > radius {
					radius = ability.Radius
				}
			case unitAbilityShieldArc:
				if e.Abilities[ai].Data > 0 && (!ability.WhenShooting || e.Shooting) && ability.Radius > radius {
					radius = ability.Radius
				}
			}
		}
		...
		dst = append(dst, unitAbilityCandidate{
			Index:  i,
			Prof:   prof,
			Radius: radius,
		})
...
func (w *World) absorbBulletByUnitAbilitiesLocked(b *simBullet, dt float32, candidates []unitAbilityCandidate) (handled bool, remove bool) {
	...
	for _, candidate := range candidates {
		if candidate.Index < 0 || candidate.Index >= len(w.model.Entities) {
			continue
		}
		e := &w.model.Entities[candidate.Index]
		...
		if candidate.Radius > 0 {
			dx := e.X - b.X
			dy := e.Y - b.Y
			r := candidate.Radius + maxf(b.Radius, 0)
			if dx*dx+dy*dy > r*r {
				continue
			}
		}
