...
func TestSaveSidecarsRelativizesWorkspaceAbsolutePaths(t *testing.T) {
	...
	for _, want := range []string{
		`assets_dir = "assets"`,
		fmt.Sprintf(`worlds_dir = %q`, filepath.Join("assets", "worlds")),
		`logs_dir = "logs"`,
		fmt.Sprintf(`directory = %q`, filepath.Join("data", "events")),
		fmt.Sprintf(`ops_file = %q`, filepath.Join("configs", "json", "ops.json")),
		fmt.Sprintf(`whitelist_file = %q`, filepath.Join("data", "state", "whitelist.json")),
	} {
	...
	wantConnUUID := fmt.Sprintf(`public_conn_uuid_file = %q`, filepath.Join("json", "conn_uuid.json"))
	if !strings.Contains(mainText, wantConnUUID) {
