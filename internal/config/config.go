...
// asLineList splits a value into one entry per non-empty line. It is used for
// values that may themselves contain commas (such as ban regexes), where CSV
// splitting would corrupt the entries.
func asLineList(v string) []string {
	v = strings.TrimSpace(v)
	if v == "" {
		return nil
	}
...
func applyINI(cfg *Config, d iniData) {
	...
	if v, ok := d.get("admin", "banned_name_regexes"); ok {
		cfg.Admin.BannedNameRegexes = asLineList(v)
	}
...
func makeINI(cfg Config) iniData {
	...
	d.set("admin", "banned_name_regexes", strings.Join(cfg.Admin.BannedNameRegexes, "\n"))
