...
func extractStatuses(src string) ([]StatusProfile, map[string]statusLookupEntry) {
	...
	for i, m := range matches {
		...
		if v, ok := lastValue(body, reDamageMultiplierStatus); ok {
			profile.DamageMultiplier = v
		}
		if v, ok := lastValue(body, reHealthMultiplierStatus); ok {
			profile.HealthMultiplier = v
		}
		if v, ok := lastValue(body, reSpeedMultiplierStatus); ok {
			profile.SpeedMultiplier = v
		}
		if v, ok := lastValue(body, reReloadMultiplierStatus); ok {
			profile.ReloadMultiplier = v
		}
		if v, ok := lastValue(body, reBuildSpeedMultiplier); ok {
			profile.BuildSpeedMultiplier = v
		}
		if v, ok := lastValue(body, reDragMultiplierStatus); ok {
