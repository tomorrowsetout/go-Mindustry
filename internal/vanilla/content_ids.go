...
func GenerateContentIDs(repoRoot, outPath string) (*ContentIDsFile, error) {
	...
	out.Blocks, err = parseBlockEntries(
		filepath.Join(base, "Blocks.java"),
		filepath.Join(repoRoot, "core", "src", "mindustry", "Vars.java"),
	)
	if err != nil {
		return nil, err
	}
...
type contentEntryEvent struct {
	offset int
	name   string
}

func parseBlockEntries(blocksPath, varsPath string) ([]ContentIDEntry, error) {
	src, err := os.ReadFile(blocksPath)
	if err != nil {
		return nil, err
	}
	maxBlockSize := parseMaxBlockSize(varsPath, 16)

	events := make([]contentEntryEvent, 0, 512)
	addMatches := func(re *regexp.Regexp, nameFn func([][]byte) []string) {
		for _, loc := range re.FindAllSubmatchIndex(src, -1) {
			if len(loc) < 2 {
				continue
			}
			submatches := make([][]byte, 0, len(loc)/2)
			for i := 0; i < len(loc); i += 2 {
				if loc[i] < 0 || loc[i+1] < 0 {
					submatches = append(submatches, nil)
					continue
				}
				submatches = append(submatches, src[loc[i]:loc[i+1]])
			}
			for _, name := range nameFn(submatches) {
				name = strings.ToLower(strings.TrimSpace(name))
				if name == "" || name == "barrier-projector" {
					continue
				}
				events = append(events, contentEntryEvent{offset: loc[0], name: name})
			}
		}
	}

	addMatches(
		regexp.MustCompile(`=\s*new\s+[A-Za-z0-9_$.]+\s*\(\s*"([^"]+)"`),
		func(matches [][]byte) []string {
			return []string{string(matches[1])}
		},
	)
	addMatches(
		regexp.MustCompile(`=\s*new\s+OreBlock\s*\(\s*Items\.([A-Za-z0-9_]+)`),
		func(matches [][]byte) []string {
			return []string{"ore-" + javaIdentifierToKebab(string(matches[1]))}
		},
	)
	addMatches(
		regexp.MustCompile(`new\s+Legacy[A-Za-z0-9_$.]*\s*\(\s*"([^"]+)"`),
		func(matches [][]byte) []string {
			return []string{string(matches[1])}
		},
	)
	addMatches(
		regexp.MustCompile(`for\s*\(\s*int\s+i\s*=\s*1\s*;\s*i\s*<=\s*Vars\.maxBlockSize\s*;\s*i\+\+\s*\)\s*\{\s*new\s+ConstructBlock\s*\(\s*i\s*\)`),
		func(matches [][]byte) []string {
			names := make([]string, 0, maxBlockSize)
			for i := 1; i <= maxBlockSize; i++ {
				names = append(names, fmt.Sprintf("build%d", i))
			}
			return names
		},
	)
	...
	out := make([]ContentIDEntry, 0, len(events))
...
func parseMaxBlockSize(path string, fallback int) int {
	if fallback <= 0 {
		fallback = 16
	}
	src, err := os.ReadFile(path)
	...
	re := regexp.MustCompile(`maxBlockSize\s*=\s*(\d+)`)
	m := re.FindSubmatch(src)
	if len(m) < 2 {
		return fallback
	}
	n, ok := parseInt16(string(m[1]))
	if !ok || n <= 0 {
		return fallback
	}
	return n
...
func javaIdentifierToKebab(s string) string {
