...
func TestLoadWorldModelFromMSAVRemapsMapLocalBlockIDsToRuntimeRegistry(t *testing.T) {
	contentChunk := buildMSAVContentChunkWithBlocks(t, []string{
		"air",
		"sand-floor",
		"copper-wall",
	...
	mapChunk := buildSingleTileMSAVMapChunk(t, 1, 0, 2)
	path := filepath.Join(t.TempDir(), "runtime-remap.msav")
	if err := os.WriteFile(path, buildMSAVWithContentAndMap(t, contentChunk, mapChunk), 0o600); err != nil {
		t.Fatalf("write test msav: %v", err)
	}
	...
	reg := protocol.NewContentRegistry()
	reg.RegisterBlock(testContent{typ: protocol.ContentBlock, id: 0, name: "air"})
	reg.RegisterBlock(testContent{typ: protocol.ContentBlock, id: 7, name: "copper-wall"})
	reg.RegisterBlock(testContent{typ: protocol.ContentBlock, id: 9, name: "sand-floor"})

	model, err := LoadWorldModelFromMSAV(path, reg)
	if err != nil {
		t.Fatalf("LoadWorldModelFromMSAV: %v", err)
	}
	tile, err := model.TileAt(0, 0)
	if err != nil || tile == nil {
		t.Fatalf("lookup decoded tile: %v", err)
	}
	if tile.Floor != 9 || tile.Block != 7 {
		t.Fatalf("expected tile to use runtime floor/block IDs 9/7, got %d/%d", tile.Floor, tile.Block)
	}
	if got := model.BlockNames[9]; got != "sand-floor" {
		t.Fatalf("expected runtime block name id 9 to be sand-floor, got %q", got)
	}
	if bytes.Equal(model.Content, contentChunk) {
		t.Fatal("expected model content header to be remapped to runtime IDs")
	}
	...
	streamContent, _, streamMap, _, _, _ := readWorldStreamCoreSections(t, payload, -1, -1, -1)
	streamNames, err := readContentBlockNames(streamContent, nil)
	if err != nil {
		t.Fatalf("read stream content names: %v", err)
	}
	if streamNames[7] != "copper-wall" || streamNames[9] != "sand-floor" {
		t.Fatalf("expected stream content header to use runtime names, got id7=%q id9=%q", streamNames[7], streamNames[9])
	}
	decoded, err := decodeMapChunk(streamMap)
	if err != nil {
		t.Fatalf("decode stream map chunk: %v", err)
	}
	decodedTile, err := decoded.TileAt(0, 0)
	if err != nil || decodedTile == nil {
		t.Fatalf("lookup stream tile: %v", err)
	}
	if decodedTile.Floor != 9 || decodedTile.Block != 7 {
		t.Fatalf("expected stream tile to keep runtime floor/block IDs 9/7, got %d/%d", decodedTile.Floor, decodedTile.Block)
	}
}
