...
func BuildWorldStreamFromMSAV(path string) ([]byte, error) {
	model, modelErr := LoadWorldModelFromMSAV(path, nil)
	...
	if len(modelPayload) > 0 && model != nil {
		if ok, matchErr := worldStreamPayloadMatchesModel(modelPayload, model); matchErr == nil && ok {
			return modelPayload, nil
		}
	}
	if len(modelPayload) > 0 {
...
build:
	...
	// CRITICAL: Java's NetworkIO.writeWorld() emits SaveIO.getSaveWriter().
	// writeDataPatches(stream, false) as the FIRST field, before the rules UTF.
	// The 159.7 client's loadWorld() calls readDataPatches() first and reads
	// exactly: int32(patchFormatVersion=2) + int32(assets.size=0 for vanilla),
	// then (for vanilla) no per-asset entries. Without this 8-byte header the
	// client misreads the rules UTF bytes as a patch header -> garbage count ->
	// IOException -> RuntimeException -> "Failed to load data!" -> disconnect
	// ("connect aborted before confirm"). This mirrors NetworkIO.writeWorld.
	w.WriteInt32(2) // DataPatcher.patchFormatVersion
	w.WriteInt32(0) // state.data.getAllAssets().size() == 0 for a vanilla session
	...
	// NOTE: the MSAV content-patches section (data.Patches) must NOT be
	// emitted into the network stream — Java's loadWorld reads the content
	// header immediately followed by the map chunk with nothing in between.
	// See buildWorldStreamFromModelState for the full rationale.
...
func worldStreamPayloadMatchesModel(payload []byte, model *world.WorldModel) (bool, error) {
	if model == nil || len(payload) == 0 {
		return false, ErrInvalidMSAV
	}
	mapChunk, err := extractMapChunkFromWorldStreamPayload(payload)
	if err != nil {
		return false, err
	}
	...
	if decoded.Width != model.Width || decoded.Height != model.Height || len(decoded.Tiles) != len(model.Tiles) {
		return false, nil
	}
...
func readMSAV(path string) (MSAVData, error) {
	...
	return readMSAVBytes(raw)
}

func readMSAVBytes(raw []byte) (MSAVData, error) {
	zr, err := zlib.NewReader(bytes.NewReader(raw))
	if err != nil {
		return MSAVData{}, err
	}
...
func readContentNamesFromRegistry(typeID byte, registry *protocol.ContentRegistry) map[int16]string {
	...
	for id, name := range snapshot[protocol.ContentType(typeID)] {
...
func inspectWorldSections(raw []byte, start int) (contentEnd, patchesEnd, mapEnd, teamBlocksLen, markersLen, customLen int, chunked bool, err error) {
	...
	// Preferred layout (matches Java NetworkIO.loadWorld): the content header is
	...
	if skipMapData(mapReader) == nil {
		...
		if teamBlocksA, _, tErr := readModernTeamBlocksRaw(teamReaderA, raw[candidateMapEnd:]); tErr == nil {
			...
			if markersA, customA, sErr := splitRawMarkersAndCustom(tailA); sErr == nil {
				...
				return contentEnd, contentEnd, candidateMapEnd, len(teamBlocksA), markersA, customA, false, nil
			}
		}
	}

	// Legacy layout: content header, then an MSAV content-patches section, then
	// the map chunk. Retained so previously generated payloads and test fixtures
	// that still carry a patches section keep parsing.
...
// errNotDataPatches signals the leading bytes are NOT a data-patches header, so
// the parser should treat the payload as starting directly with the rules UTF.
var errNotDataPatches = errors.New("not a data-patches header")

// skipDataPatchesHeader advances r past the leading data-patches header that Java's
// NetworkIO.writeWorld() emits first (SaveIO.getSaveWriter().writeDataPatches).
// Wire layout (SaveVersion.writeDataPatches / readDataPatches):
//
//	int32 patchFormatVersion(=2)            // ignored by the client
//	int32 assetCount
//	assetCount * {
//	    byte  typeId
//	    UTF   path
//	    bool  embedded
//	    if embedded: asset.read(stream)     // variable-length, asset-type specific
//	    else:          32-byte SHA-256 hash
//	}
//
// Vanilla join payloads carry assetCount == 0, so the header is exactly 8 bytes.
// Returns errNotDataPatches when the leading int32 is not the patch format version.
func skipDataPatchesHeader(r *javaReader) error {
	version, err := r.ReadInt32()
	if err != nil {
		return err
	}
	if version != 2 {
		return errNotDataPatches
	}
	count, err := r.ReadInt32()
	if err != nil {
		return err
	...
	if count < 0 || count > 1<<20 {
		return fmt.Errorf("data-patches asset count out of range: %d", count)
	}
	for i := int32(0); i < count; i++ {
		if _, err := r.ReadByte(); err != nil { // typeId
			return err
		}
		if _, err := r.ReadUTF(); err != nil { // path
			return err
		}
		embedded, err := r.ReadByte()
		if err != nil {
			return err
		}
		if embedded != 0 {
			// Embedded asset payloads are variable-length and asset-type specific;
			// the vanilla server never emits them, so a generic skip is not implemented.
			return fmt.Errorf("embedded data-patch asset unsupported by header skip")
		}
		if err := r.Skip(32); err != nil { // SHA-256 hash
			return err
		}
	}
	return nil
...
func locatePlayerStart(raw []byte) (int, error) {
	...
	body := raw
	if len(raw) >= 8 {
		// Skip the leading data-patches header if present. It is detected by the
		// constant leading int32 (patchFormatVersion == 2), which can never collide
		// with a real rules UTF (rules JSON always starts with '{', so its
		// length-prefixed modified-UTF8 never begins with 00 00 00 02).
		probe := newJavaReader(raw)
		if herr := skipDataPatchesHeader(probe); herr == nil {
			headerLen = probe.Offset()
			body = raw[headerLen:]
		} else if herr != errNotDataPatches {
			return 0, herr
		}
	}

	r := newJavaReader(body)
...
// splitDataPatchesHeader separates the leading data-patches header (if present)
// from the rest of a decompressed NetworkIO.writeWorld payload. The header is
// detected by its constant leading int32 (patchFormatVersion == 2). When present,
// it returns (header, body, nil); otherwise header is nil and body == raw.
func splitDataPatchesHeader(raw []byte) (header, body []byte, err error) {
	if len(raw) < 8 {
		return nil, raw, nil
	}
	probe := newJavaReader(raw)
	herr := skipDataPatchesHeader(probe)
	if herr == nil {
		n := probe.Offset()
...
// reattachHeader recompresses (header + body) into a zlib world-stream payload,
// preserving the leading data-patches header verbatim.
func reattachHeaderAndCompress(header, body []byte) ([]byte, error) {
...
func RewritePlayerIDInWorldStream(payload []byte, playerID int32) ([]byte, error) {
	...
	header, body, herr := splitDataPatchesHeader(raw)
	if herr != nil {
		return nil, herr
	}
	...
	binary.BigEndian.PutUint32(body[idPos:idPos+4], uint32(playerID))

	return reattachHeaderAndCompress(header, body)
}
...
func RewriteRulesInWorldStream(payload []byte, rules string) ([]byte, error) {
	...
	patched := make([]byte, 0, len(header)+head.Len()+len(body)-oldRulesEnd)
	patched = append(patched, header...)
	patched = append(patched, head.Bytes()...)
	patched = append(patched, body[oldRulesEnd:]...)
...
func RewriteRuntimeStateInWorldStream(payload []byte, wave int32, wavetimeTicks float32, tick float64, playerID int32) ([]byte, error) {
	...
	if wavePos+4 > len(body) || wavetimePos+4 > len(body) || tickPos+8 > len(body) || playerIDPos+4 > len(body) {
		return nil, io.ErrUnexpectedEOF
	}
	binary.BigEndian.PutUint32(body[wavePos:wavePos+4], uint32(wave))
	binary.BigEndian.PutUint32(body[wavetimePos:wavetimePos+4], math.Float32bits(wavetimeTicks))
	binary.BigEndian.PutUint64(body[tickPos:tickPos+8], math.Float64bits(tick))
	binary.BigEndian.PutUint32(body[playerIDPos:playerIDPos+4], uint32(playerID))
