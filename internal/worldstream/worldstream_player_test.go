...
func contentStartFromWorldStreamRaw(raw []byte) (int, int, error) {
	// Production world-stream payloads now carry a leading data-patches header
	// (Java NetworkIO.writeWorld -> writeDataPatches). Skip it so offsets stay
	// absolute into the full raw buffer. Headerless payloads are unaffected.
	header, body, err := splitDataPatchesHeader(raw)
	if err != nil {
		return 0, 0, err
	}
	...
	playerStart := headerLen + r.Offset()
	...
	return playerStart, headerLen + r.Offset(), nil
}
...
func TestBuildWorldStreamFromMSAVPreservesWorldSections(t *testing.T) {
	...
	// The MSAV content-patches section must NOT be emitted into the network
	// stream: Java's loadWorld reads the content header immediately followed by
	// the map chunk with nothing in between. Carrying patch bytes here would
	// shift the client's map parse (ClassCastException on floor cast).
	if len(patches) != 0 {
		t.Fatalf("expected no content-patches section in the network stream, got %d bytes", len(patches))
	}
	decoded, err := decodeMapChunk(mapChunk)
	if err != nil {
...
func TestRewriteRulesInWorldStreamReplacesLeadingRulesBlob(t *testing.T) {
	...
	// The rewritten payload preserves the leading data-patches header; skip it
	// to read the first real field (rules).
	_, body, err := splitDataPatchesHeader(raw)
	if err != nil {
		t.Fatalf("split data-patches header: %v", err)
	}
	r := newJavaReader(body)
...
func TestBuildWorldStreamFromModelSnapshotPreservesCustomAndOmitsPatches(t *testing.T) {
	...
	// Content-patches is an MSAV-file-internal section; it must not appear in
	// the join-world network stream (Java loadWorld has no section for it).
	if len(patches) != 0 {
		t.Fatalf("expected no content-patches section in the network stream, got %v", patches)
	}
...
// TestNetworkStreamContentImmediatelyPrecedesMap locks in the wire-layout
// invariant that crashed the Java client when violated: after the content
// header, the map chunk must begin with ZERO intervening bytes. Java's
// loadWorld reads readContentHeader(stream) then readMap(stream) back to back.
// Any stray section (e.g. the MSAV content-patches blob that used to be
// emitted here) shifts the entire map parse and makes the client read garbage
// floor IDs -> ClassCastException "cannot be cast to Floor".
func TestNetworkStreamContentImmediatelyPrecedesMap(t *testing.T) {
	path := filepath.Join("..", "..", "assets", "worlds", "file.msav")
	payload, err := BuildWorldStreamFromMSAV(path)
	if err != nil {
		t.Fatalf("build world stream: %v", err)
	...
	if chunked {
		t.Fatal("expected raw (non-chunked) network layout")
	}
	// patchesEnd == contentEnd is the assertion that there is no content-patches
	// section between the content header and the map chunk.
	if patchesEnd != contentEnd {
		t.Fatalf("expected content header to be immediately followed by the map chunk (patchesEnd %d must equal contentEnd %d)", patchesEnd, contentEnd)
	}
	if mapEnd <= contentEnd {
		t.Fatalf("expected a non-empty map chunk after the content header (mapEnd %d, contentEnd %d)", mapEnd, contentEnd)
	}
}
