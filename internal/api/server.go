...
// Route registers an additional HTTP handler on the API mux. Handlers are
// wrapped with the same API-key auth middleware as the built-in endpoints.
func (s *Server) Route(pattern string, handler http.HandlerFunc) error {
	if s == nil || s.mux == nil {
		return errors.New("api server not initialized")
	}
	if strings.TrimSpace(pattern) == "" {
		return errors.New("api route pattern is empty")
	}
	if handler == nil {
		return errors.New("api route handler is nil")
	}
	s.mux.HandleFunc(pattern, s.withAuth(handler))
	return nil
}
